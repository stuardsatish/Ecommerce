...
    // Most channels start "placed"; the billing (offline POS) flow overrides
    // this to "delivered" since it's an in-store, hand-to-customer sale.
    orderStatus:   pending.orderStatus ?? "placed",
    // GST breakdown (stored so invoices stay accurate if rates change later).
...
  // Billing (offline POS) manual discount fields — recorded for the invoice/audit
  if (pending.manualDiscount != null) orderDoc.manualDiscount     = pending.manualDiscount;
  if (pending.manualDiscountType)     orderDoc.manualDiscountType = pending.manualDiscountType;
