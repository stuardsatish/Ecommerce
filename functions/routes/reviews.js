...
router.post("/create", async (req, res) => {
  try {
    ...
    if (await isFeatureKilled("reviews")) {
      return res
        .status(503)
        .json({ success: false, error: "Reviews are temporarily disabled." });
    }
    if (await isUserBlocked(userId)) {
      return res
        .status(403)
        .json({ success: false, error: "Your account cannot post reviews." });
    }

    if (
      !(await rateLimit(`review:${userId}`, 10, 60000, { failOpen: false }))
    ) {
      return res
        .status(429)
        .json({
    ...
    if (!Number.isFinite(r) || r < 1 || r > 5)
      return res
        .status(400)
        .json({ success: false, error: "Rating must be between 1 and 5" }); // Purchase check: any of this user's orders must contain the product.
    ...
    const purchased = ordersSnap.docs.some((d) => {
      ...
      return (
        ...
        products.some((p) => String(p.productId) === pid)
      );
    });
    if (!purchased) {
      return res
    ...
    return res
      .status(500)
      .json({ success: false, error: "Could not submit review" });
  }
});
