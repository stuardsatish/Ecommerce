  ...
  try {
    ...
    if (await isUserBlocked(userId)) {
      return res
        ...
        .json({
          success: false,
          error:
            "Your account is not able to place orders. Please contact support.",
        });
    } // Per-user limit (fail-CLOSED: limiter outage must not open the floodgates).

    if (
      !(await rateLimit(`create-order:${userId}`, 15, 60000, {
        failOpen: false,
      }))
    ) {
    ...
    if (!(await globalDailyLimit("orders", ORDERS_DAILY_CAP, utcDay))) {
      return res
        ...
        .json({
          success: false,
          error:
            "The store is experiencing very high volume. Please try again later.",
        });
    }
    ...
    if (promoCode && typeof promoCode === "string" && promoCode.trim()) {
      ...
      try {
        ...
        if (!promoSnap.empty) {
          const pd = promoSnap.docs[0].data(); // Expiry check
        ...
        console.warn("[payment] promo lookup failed:", promoErr); // Non-fatal: proceed without the promo discount.
      }
    } // --------------------------------------------------
    ...
    try {
      ...
      if (shippingSnap.exists) {
        ...
        if (typeof sd.freeShippingThreshold === "number")
          shippingThreshold = sd.freeShippingThreshold;
        if (typeof sd.shippingCost === "number") shippingFee = sd.shippingCost;
      }
    } catch (shipErr) {
      console.warn(
        "[payment] shipping settings lookup failed, using defaults:",
        shipErr,
      );
    }
    ...
    try {
      ...
      console.warn("[payment] buyer state lookup failed:", e);
    }
    try {
      const setSnap = await db()
        .collection("settings")
        .doc("invoiceSettings")
        .get();
      if (setSnap.exists) sellerState = setSnap.data()?.state || "";
    } catch (e) {
      console.warn("[payment] seller state lookup failed:", e);
    }
...
router.post("/cod-create", async (req, res) => {
  try {
    ...
    if (
      !(await rateLimit(`cod-create:${userId}`, 15, 60000, { failOpen: false }))
    ) {
      return res
        .status(429)
        .json({
    ...
    const fdb = db(); // 1. COD must be enabled in settings/paymentSettings.

    const paySnap = await fdb
      .collection("settings")
      .doc("paymentSettings")
      .get();
    ...
    if (!street || !String(street).trim()) {
      return res.status(400).json({
        success: false,
        error:
          "Delivery address is required for COD orders. Please update your profile address.",
      });
    ...
    if (promoCode && typeof promoCode === "string" && promoCode.trim()) {
      ...
      try {
        ...
        if (!promoSnap.empty) {
          ...
          const isExpired =
            pd.expiryDate && new Date(pd.expiryDate).getTime() < Date.now();
          if (!isExpired) {
            const value = Number(pd.value || 0);
            if (pd.type === "percent")
              promoDiscount = Math.round((subtotal * value) / 100);
    ...
    try {
      ...
      console.warn(
        "[payment] cod shipping settings lookup failed, using defaults:",
        shipErr,
      );
    }
    const shipping = subtotal >= shippingThreshold ? 0 : shippingFee;
    ...
    let sellerState = "";
    try {
      buyerState = userData?.address?.state || "";
    } catch (e) {
      /* string address → no state */
    }
    try {
      const setSnap = await fdb
        .collection("settings")
        .doc("invoiceSettings")
        .get();
      if (setSnap.exists) sellerState = setSnap.data()?.state || "";
    } catch (e) {
      console.warn("[payment] cod seller state lookup failed:", e);
    }
    ...
    const userAddress =
      typeof addr === "object"
        ? `${addr.street || ""}, ${addr.city || ""}, ${addr.state || ""} ${addr.pincode || ""}`
            ...
            .replace(/^[,\s]+|[,\s]+$/g, "")
            .trim()
        : addr; // 8. Write the order atomically via the shared single-source-of-truth writer.

    const orderId = await fdb.runTransaction(async (tx) => {
      const productRefs = gstItems.map((it) =>
      ...
      const customerSnap = await tx.get(customerRef); // Re-validate stock at write time.
    ...
    return res
      ...
      .json({
        success: false,
        error: err.message || "Failed to place COD order",
      });
  }
});
...
router.post("/verify", async (req, res) => {
  try {
    ...
    if (
      !(await rateLimit(`verify:${decoded.uid}`, 30, 60000, {
        failOpen: false,
      }))
    ) {
      return res.status(429).json({
...
async function finalizeOrder(razorpayOrderId, razorpayPaymentId, expectedUid) {
  ...
  return fdb.runTransaction(async (tx) => {
    ...
    const customerSnap = await tx.get(customerRef); // Re-validate stock at finalize time.
