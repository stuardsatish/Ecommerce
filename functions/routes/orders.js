...
function parseWhatsAppMessage(text) {
  ...
  const pick = (line) => line.slice(line.toLowerCase().indexOf(":") + 1).trim(); // Keep only digits and the decimal point — strips ₹, Rs., commas, spaces.

  const parsePrice = (str) =>
    parseFloat(String(str).replace(/[^-0-9.]/g, "")) || 0; // ── header fields ─────────────────────────────────────────────────────
  ...
  for (const line of lines) {
    ...
    else if (/^total items\s*:/.test(lc))
      result.totalItems = parseInt(pick(line), 10) || 0;
    ...
    } else if (/^promo discount\s*:/.test(lc))
      result.promoDiscount = parsePrice(pick(line));
    else if (/^total amount\s*:/.test(lc))
      result.totalAmount = parsePrice(pick(line));
  } // ── items ──────────────────────────────────────────────────────────────
    ...
    if (match) {
      ...
      while (j < lines.length) {
        ...
        if (nl.startsWith("quantity:")) {
          quantity =
            parseInt(lines[j].slice(lines[j].indexOf(":") + 1).trim(), 10) || 0;
          j++;
          ...
          discountedPrice = parsePrice(
            lines[j].slice(lines[j].indexOf(":") + 1),
          );
          j++;
      ...
      if (title && quantity > 0) {
        result.items.push({
          title,
          quantity, // originalPrice = MRP; finalPrice = discounted price
  ...
  try {
    ...
    if (await isFeatureKilled("orders")) {
      return res
        ...
        .json({
          success: false,
          error: "Order creation is temporarily disabled.",
        });
    ...
    if (!message || typeof message !== "string" || !message.trim()) {
      return res
        .status(400)
        .json({ success: false, error: "Order message is required" });
    } // Only "paid" or "pending" are valid values

    const safeStatus = paymentStatus === "pending" ? "pending" : "paid"; // ── 1. Parse ──────────────────────────────────────────────────────────
    ...
    if (!parsed.items.length)
      return res
        .status(400)
        .json({ success: false, error: "No items found in the message." });
    ...
    if (!parsed.customerId && !parsed.email)
      return res
        ...
        .json({
          success: false,
          error: "Could not find Customer ID or Email in the message.",
        });
    ...
    if (!parsed.totalAmount)
      return res
        ...
        .json({
          success: false,
          error: "Could not extract Total Amount from the message.",
        });

    const fdb = db(); // ── 2. Resolve customer ──────────────────────────────────────────────

    let userId,
      userName,
      userEmail,
      userPhone,
      isGuest = false; // Prefer Customer ID (sent by CartPage.jsx — most reliable)
    ...
    for (const item of parsed.items) {
      const needle = item.title.toLowerCase().trim(); // 1st pass: exact match (case-insensitive)
      ...
      if (!matches.length) {
        ...
        const p = matches[0]; // SERVER-AUTHORITATIVE pricing: always price from the live catalog.
    ...
    const serverSubtotal =
      Math.round(
        resolvedItems.reduce((s, it) => s + it.finalPrice * it.quantity, 0) *
          100,
      ) / 100;
    ...
    const serverTotal = Math.max(
      0,
      Math.round(serverSubtotal + msgShipping - msgPromoDiscount),
    );
    ...
    const recordedTotal = priceMismatch ? pastedTotal : serverTotal; // Seller state for the GST CGST/SGST-vs-IGST split (read before the tx).

    const sellerState = await getSellerState(); // ── 4. Transaction: stock validation + all writes ─────────────────────
    ...
    const orderId = await fdb.runTransaction(async (tx) => {
      ...
      const userData = userDocSnap.exists
        ? userDocSnap.data()
        : { name: userName, email: userEmail }; // Stock validation
      ...
      const totalItems = resolvedItems.reduce((s, it) => s + it.quantity, 0); // GST breakdown — buyer state from the resolved user doc.
    ...
    console.error("[orders] manual-create failed:", err); // STOCK errors carry an admin-actionable message; everything else is a
...
router.post("/billing-create", async (req, res) => {
  try {
    ...
    } = req.body || {}; // ── 1. Validate inputs ────────────────────────────────────────────────

    if (!Array.isArray(items) || items.length === 0) {
      return res
        .status(400)
        .json({ success: false, error: "No items in the bill." });
    }
    const safeMethod = VALID_PAYMENT_METHODS.includes(paymentMethod)
      ? paymentMethod
      : "Cash";
    const safeStatus = paymentStatus === "pending" ? "pending" : "paid";
    const safeDiscType = manualDiscountType === "percent" ? "percent" : "flat";

    const fdb = db(); // ── 2. Server-authoritative pricing (catalog is source of truth) ──────
    ...
    for (const it of items) {
      ...
      if (!pid || qty <= 0) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid bill item." });
      }
      ...
      if (!snap.exists) {
        return res
          .status(400)
          .json({ success: false, error: `Product not found: ${pid}` });
      }
      const p = snap.data();
    ...
    const subtotalRounded = Math.round(subtotal * 100) / 100; // ── 3. Promo code (validated server-side; client sends only the code) ──
    ...
    if (promoCode && typeof promoCode === "string" && promoCode.trim()) {
      ...
      try {
        ...
        if (!promoSnap.empty) {
          ...
          const expired =
            pd.expiryDate && Date.now() > new Date(pd.expiryDate).getTime();
          if (!expired) {
            const value = Number(pd.value || 0);
            if (pd.type === "percent")
              promoDiscount = Math.round((subtotalRounded * value) / 100);
    ...
    const total = Math.max(
      0,
      Math.round(subtotalRounded - promoDiscount - manualDiscountAmount),
    );
    if (total <= 0) {
      return res
        .status(400)
        .json({
          success: false,
          error: "Bill total must be greater than zero.",
        });
    ...
    if (!userId) {
      isGuest = true;
      const key = (phone || email || String(Date.now())).replace(
        /[^a-zA-Z0-9]/g,
        "_",
      );
      userId = `walkin_${key}`;
    ...
    const isInterState = resolveInterState(buyerState, sellerState);
    const gstItems = applyGstToItems(resolvedItems, isInterState);
    const gstTotals = sumGstTotals(gstItems); // ── 7. Transaction: stock validation + all writes (shared writer) ─────
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to create bill" });
  }
});
