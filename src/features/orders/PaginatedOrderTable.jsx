 ...
 */
export default function PaginatedOrderTable({
  orders,
  search,
  active = true,
  onActivePageChange,
  ...
  const { pagedOrders, total, page, setPage, pageSize, setPageSize } =
    useOrderFilter(orders, search);
    ...
    if ((orders?.length || 0) > 0) {
      return (
        <div
          ...
          style={{
            border:
              "2px dashed color-mix(in srgb, var(--color-border) 50%, transparent)",
            borderRadius: "32px",
            padding: "64px 32px",
            background:
        ...
        >
          <SearchIcon
            size={40}
            style={{ color: "var(--color-body)", opacity: 0.3 }}
          />
          <p
            style={{
