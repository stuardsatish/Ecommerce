...
export async function generateInvoice(order, userDetails = {}) {
  if (!order) return; // Fetch customizable settings from Firestore or fall back to defaults
  ...
  const website = settings.website || ""; // Pre-load company logo image if URL is configured
  ...
  const rightX = pageW - M; /* ---- identifiers ---- */

  const rawId = order.externalOrderId
  ...
  const customerName = clean(
    userDetails.name || order.userName,
    "Guest Customer",
  ...
  const round2 = (n) => Math.round((Number(n) || 0) * 100) / 100; // Whether this order carries server-computed GST, and whether it's an
  ...
  const interState = hasStoredGst
    ? !!order.isInterState
    : !!userDetails.state &&
      clean(userDetails.state, "").toLowerCase() !==
        (settings.state || COMPANY.state).toLowerCase();
  ...
  const lines = products.map((p, i) => {
    ...
    const discPct =
      discAmt > 0
        ? Number(p.discount) > 0
          ...
          : `${Math.round((discAmt / mrp) * 100)}%`
        : "0%";
    const lineTotal = unit * qty;
    itemsTotal += lineTotal; // Per-product GST rate (legacy orders fall back to the old global rate).
    const gstRate = p.gstRate != null ? Number(p.gstRate) : LEGACY_GST_RATE; // Prefer the taxable value stored with the order (per unit); otherwise
    // back-calculate from the GST-inclusive unit price.
    const taxableUnit =
      Number(p.taxableValue) ||
      (gstRate > 0 ? (unit * 100) / (100 + gstRate) : unit);
    const taxable = taxableUnit * qty;
    const lineGst = round2((unit - taxableUnit) * qty); // Per-item CGST/SGST/IGST — prefer stored amounts, else derive from the
    ...
    if (p.cgstAmount != null || p.sgstAmount != null || p.igstAmount != null) {
      ...
      cgst = Math.ceil((lineGst / 2) * 100) / 100;
      sgst = Math.floor((lineGst / 2) * 100) / 100;
      igst = 0;
    }
    const name =
      discAmt > 0
  ...
  if (body.length === 0) {
    ...
    body.push(
      interState
        ? [...empty, money(0), money(0)]
        : [...empty, money(0), money(0), money(0)],
    );
  ...
  const derivedShipping = Math.max(
    0,
    Math.round((grand - itemsTotal + promoDiscount) * 100) / 100,
  );
  const shipping = storedShipping > 0 ? storedShipping : derivedShipping;
  const goodsValue = itemsTotal; // GST summary — prefer the totals stored with the order (per-product, computed
  ...
  const separatorY = Math.max(textY + 4, M + 64); // draw separator line

  doc.setDrawColor(199, 196, 215);
  doc.line(M, separatorY, rightX, separatorY); // "TAX INVOICE" label (right)
  ...
  doc.text(`Order Date: ${orderDate}`, rightX, M + 52, {
    align: "right",
  }); /* ============================== BILL TO / SHIP TO ============================== */ // Orders persist the delivery address as `userAddress` (string OR object);
  ...
  const billLines = [
    ...
    emailVal,
    phoneVal,
    ...doc.splitTextToSize(addr, colGap - 16),
  ];
  const shipLines = [
    customerName,
    phoneVal,
  ...
  y +=
    16 +
    Math.max(billLines.length, shipLines.length) * 13 +
    12; /* ============================== ITEMS TABLE ============================== */ // Columns adapt to the sale type: inter-state shows a single IGST column,
  ...
  }); /* ============================== TAX SUMMARY (by HSN / rate) ============================== */ // Clean pagination for everything drawn after the (auto-paginating) items
  // table: if a block won't fit above the bottom margin, start a fresh page.

  const pageBottom = pageH - M;
  const ensure = (yy, need) =>
    yy + need > pageBottom ? (doc.addPage(), M) : yy;
  ...
  const sumTax = taxGroups.reduce(
    (a, g) => ({
      taxable: a.taxable + g.taxable,
      cgst: a.cgst + g.cgst,
      sgst: a.sgst + g.sgst,
      igst: a.igst + g.igst,
    }),
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      if (
        data.section === "body" &&
        data.row.index === (taxBody.length ? taxBody.length : 0)
      ) {
        data.cell.styles.fontStyle = "bold";
      }
    },
  }); /* ============================== SUMMARY + PAYMENT ============================== */ // Keep the summary (right) and payment (left) blocks together on one page.

  const txnId = clean(order.razorpayPaymentId, "");
  const summaryRows =
    1 + (interState ? 1 : 2) + 1 + (promoDiscount > 0 ? 1 : 0);
  const summaryHeight = summaryRows * 14 + 18 + 8;
  const paymentHeight = txnId ? 48 : 32;
  const blockTop = ensure(
    doc.lastAutoTable.finalY + 18,
    Math.max(summaryHeight, paymentHeight),
  );
  ...
  doc.setDrawColor(199, 196, 215);
  doc.line(labelX, sy - 4, valX, sy - 4);
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(70, 68, 85); // Friendly wording for Cash-on-Delivery orders.
  const payMethodStr =
  ...
  const payBottom = txnId
    ? py + 32
    : py +
      16; /* ============================== FOOTER ============================== */ // Footer needs ~100pt; if it won't fit under the blocks, move to a new page.

  let fy = ensure(Math.max(sy, payBottom) + 40, 100); // dotted separator
  doc.setLineDashPattern([2, 2], 0);
  doc.setDrawColor(199, 196, 215);
  doc.line(M, fy, rightX, fy);
  doc.setLineDashPattern([], 0);

  let footerY = fy + 20; // Footer Title (Default "Thank You!" if settings empty or not set)
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(70, 68, 85); // Footer Sub Note (if available)
