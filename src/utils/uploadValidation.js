...
export function validateImageFile(file, opts = {}) {
  ...
  const ext = (file.name.split(".").pop() || "").toLowerCase(); // Block SVG by type OR extension (defence in depth).
  ...
  const allowedTypes = allowGif
    ? ALLOWED_TYPES
    : ALLOWED_TYPES.filter((t) => t !== "image/gif");
  const allowedExts = allowGif
    ? ALLOWED_EXTENSIONS
    : ALLOWED_EXTENSIONS.filter((e) => e !== "gif");
