...
const Navbar = () => {
  ...
  const wishlistItems = useSelector((state) => state.wishlist.wishlistItems); // Admins don't use the storefront — hide Products/Process/Contact + cart/wishlist.

  const isAdmin = user?.role === "admin";
  const adminHome = "/admin/myorders"; // Admins get Home (their dashboard) + the two analytics dashboards inline.
  const visibleNavItems = isAdmin
    ? [
        ...
        { name: "Users Analytics", path: "/admin/allUsersOrdersAnalytics" },
        {
          ...
          path: "/admin/allProductsOrdersAnalytics",
        },
      ]
    : navItems;
  ...
  const [lastScrollY, setLastScrollY] = useState(0); // Special route logic: Products route (/products) stays visible
  ...
  const navHidden = false; // Navbar background adapts to the current page so it always blends in,
  // and we paint the body the same color (covers the gap around the pill).

  const bgKey = Object.keys(ROUTE_BG).find((p) =>
  ...
  useEffect(() => {
    ...
    fetchUser();
  }, [user]); // Click outside to close menus
  ...
  }, [location.pathname]); // Close mobile drawer on route change — and always release the body scroll
  ...
  }, [location.pathname]); // Lock body scroll when drawer is open
  ...
  }, [drawerOpen]); // Keep the floating-nav pill styling on at all times (navbar no longer
  // auto-hides on scroll — it stays pinned in the header area).

  useEffect(() => {
    navContainerRef.current?.classList.add("floating-nav");
    setLastScrollY(currentScrollY);
  }, [currentScrollY]); // GSAP Animation — the navbar is permanently visible; it only retracts on the
  ...
  const handleLogout = useCallback(async () => {
    try {
      ...
      if (current) {
        try {
          await updateDoc(doc(fireDB, "users", current.uid), {
...
          Desktop only — the mobile shell (MobileNav) replaces the navbar below md. */}
      ...
      >
        ...
        <header className="absolute top-1/2 w-full -translate-y-1/2">
                   {" "}
          <nav
            className="relative flex size-full items-center justify-between p-4 backdrop-blur-sm rounded-lg sm:rounded-2xl border border-ink/10 shadow-sm"
              ...
              <div className="hidden md:flex items-center gap-5 ml-8">
                ...
                >
                  ...
                  <span className="">
                                       {" "}
                    {darkMode ? (
                      <FaSun size={18} style={{ color: navText }} />
                    ) : (
                      <FaMoon size={18} style={{ color: navText }} />
                    )}
                                     {" "}
                  </span>
                                 {" "}
                      ...
                      {user && user.role === "admin" ? (
                        <>
                          ...
                          <NavLink
                            ...
                            className="block px-4 py-2 text-sm text-ink hover:bg-surface-inverse/5"
                          >
                            Users
                          </NavLink>
                                                   {" "}
                          ...
                          <button
                            ...
                            className="w-full text-left px-4 py-2 text-sm text-error hover:bg-error-subtle border-t border-ink/5"
                          >
                            Logout
                          </button>
                                                 {" "}
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-6 border-b border-ink/5">
            ...
            >
                           {" "}
              {darkMode ? (
                <FaSun size={18} className="text-ink" />
              ) : (
                <FaMoon size={18} className="text-ink" />
              )}
                         {" "}
            </button>
                     {" "}
              ...
              <div className="space-y-1">
                ...
                {user.role === "admin" ? (
                  <>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/admin/users")}
                      className={drawerItemClass("/admin/users")}
                    >
                      Users
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/admin/add-product")}
                      className={drawerItemClass("/admin/add-product")}
                    >
                      Add Product
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/admin/createOrders")}
                      className={drawerItemClass("/admin/createOrders")}
                    >
                      WhatsApp Order
                    </button>
                                       {" "}
                    <button
                      ...
                      className={drawerItemClass("/admin/billing")}
                    >
                      Billing
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/admin/optimization")}
                      className={drawerItemClass("/admin/optimization")}
                    >
                      Optimization
                    </button>
                                       {" "}
                    <button
                      onClick={() =>
                        handleDrawerNav("/admin/adminUploadOrders")
                      }
                      className={drawerItemClass("/admin/adminUploadOrders")}
                    >
                      Upload Orders
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/admin/myorders")}
                      className={drawerItemClass("/admin/myorders")}
                    >
                      My Orders
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/admin/add-discount")}
                      className={drawerItemClass("/admin/add-discount")}
                    >
                      Add Discount
                    </button>
                                     {" "}
                  ...
                  <>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/myprofile")}
                      className={drawerItemClass("/myprofile")}
                    >
                      My Profile
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/userorders")}
                      className={drawerItemClass("/userorders")}
                    >
                      Current Orders
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/userpastorders")}
                      className={drawerItemClass("/userpastorders")}
                    >
                      Past Orders
                    </button>
                                       {" "}
                    <button
                      onClick={() => handleDrawerNav("/wishlist")}
                      className={drawerItemClass("/wishlist")}
                    >
                      Wishlist
                    </button>
                                     {" "}
