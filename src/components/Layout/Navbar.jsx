...
const ROUTE_BG = {
  "/admin/testing": "#0f172a",
  "/admin/add-product": "#F2F3F6",
  "/admin/myorders": "#FBF9F8",
  "/admin/users": "#FBF9F8",
  "/admin/createOrders": "#FBF9F8",
  "/admin/optimization": "#FBF9F8",
  "/admin/adminUploadOrders": "#F8F7F4",
  "/admin/allUsersOrdersAnalytics": "#F2F4F8",
  "/admin/allProductsOrdersAnalytics": "#FAF8FF",
};
const DEFAULT_BG = "#FCF9F8";
const isDarkBg = (hex) => {
  const c = hex.replace("#", "");
...
const Navbar = () => {
  ...
  const adminHome = "/admin/add-product";
  ...
  const navText = navDark ? "#F6F3F2" : "#000000";
  ...
  const drawerItemClass = (path) =>
    clsx(
      ...
      location.pathname === path || location.pathname.startsWith(path + "/")
        ...
        : "text-black/70 font-medium hover:text-black"
    );
    ...
    <>
      ...
      >
        <header className="absolute top-1/2 w-full -translate-y-1/2">
          <nav className="relative flex size-full items-center justify-between p-4 backdrop-blur-sm rounded-lg sm:rounded-2xl border border-black/10 shadow-sm" style={{ background: navBg }}>
            ...
            <div className="flex items-center gap-7">
              ...
              {!isAdmin && (
                <button
                  ...
                  className="bg-transparent border border-black/20 text-black md:flex hidden items-center justify-center gap-2 px-4 py-2 rounded-full font-bold text-xs uppercase transition-transform hover:scale-105 hover:bg-black/5"
                >
                  <span>Products</span>
                  <TiLocationArrow />
                </button>
            ...
            <div className="flex h-full items-center">
              ...
              <div className="hidden md:flex items-center gap-5 ml-8">
                ...
                <button
                  ...
                  className="flex items-center group p-2 hover:bg-black/5 rounded-full transition-all"
                  title="Toggle Theme"
                >
                  <span className="">
                    {darkMode ? <FaMoon size={18} style={{ color: navText }} /> : <FaSun size={18} style={{ color: navText }} />}
                  </span>
                </button>
                ...
                {user && !isAdmin && (
                  <>
                    <button
                      ...
                      className="relative text-black hover:text-orange-600 transition-colors"
                    >
                      <FaHeart size={18} />
                      {wishlistCount > 0 && (
                ...
                <div className="relative" ref={menuRef}>
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 mt-4 w-56 bg-white/95 backdrop-blur-xl border border-black/10 rounded-xl overflow-hidden shadow-2xl z-[60]">
                      <div className="px-4 py-3 border-b border-black/5">
                        <p className="text-[10px] uppercase tracking-widest text-gray-400">
                          {user ? "Signed in as" : "Welcome"}
                        </p>
                        <p className="text-sm font-bold text-black truncate">
                          {user ? userData?.name || "User" : "Guest"}
                        </p>
                      </div>
                      ...
                      {!user && (
                        <button
                          ...
                          className="w-full text-left px-4 py-3 text-sm text-black hover:bg-black/5 font-medium"
                        >
                          Login
                        </button>
                      )}
                      ...
                      {user && user.role === "admin" ? (
                        <>
                          <NavLink to="/admin/users" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Users</NavLink>
                          <NavLink to="/admin/add-product" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Add Product</NavLink>
                          <NavLink to="/admin/createOrders" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">WhatsApp Order</NavLink>
                          <NavLink to="/admin/optimization" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Optimization</NavLink>
                          <NavLink to="/admin/adminUploadOrders" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Upload Orders</NavLink>
                          <NavLink to="/admin/myorders" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">My Orders</NavLink>
                          <NavLink to="/admin/add-discount" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Add Discount</NavLink>
                          <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 border-t border-black/5">Logout</button>
                        </>
                      ) : user ? (
                        <>
                          <NavLink to="/myprofile" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">My Profile</NavLink>
                          <NavLink to="/userorders" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Current Orders</NavLink>
                          <NavLink to="/userpastorders" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Past Orders</NavLink>
                          <NavLink to="/wishlist" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-sm text-black hover:bg-black/5">Wishlist</NavLink>
                          <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 border-t border-black/5">Logout</button>
                        </>
                      ) : null}
                    </div>
              ...
              <div className="flex md:hidden items-center gap-3">
                {user && !isAdmin && (
                  <>
                    <button
                      onClick={() => navigate("/wishlist")}
                      className="relative text-black min-w-[44px] min-h-[44px] flex items-center justify-center"
                    >
                      <FaHeart size={18} />
                      {wishlistCount > 0 && (
                        <span className="absolute top-1 right-1 bg-orange-600 text-white text-[9px] rounded-full w-4 h-4 flex items-center justify-center">
                          {wishlistCount}
                        </span>
                      )}
                    </button>
                    ...
                    >
                      ...
                      {totalCount > 0 && (
                        <span className="absolute top-1 right-1 bg-black text-white text-[9px] rounded-full w-4 h-4 flex items-center justify-center">
                          {totalCount}
                        </span>
                      )}
                    </button>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-6 border-b border-black/5">
            <NavLink to={isAdmin ? adminHome : "/"} onClick={() => setDrawerOpen(false)} className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold uppercase tracking-widest text-black">Menu</span>
            </NavLink>
            <button
              onClick={() => dispatch(toggleTheme())}
              className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full hover:bg-black/5 transition-all"
            >
              {darkMode ? <FaMoon size={18} className="text-black" /> : <FaSun size={18} className="text-black" />}
            </button>
          </div>
            ...
            {user ? (
              <div className="space-y-1">
                <div className="px-0 py-3 mb-2">
                  ...
                  <p className="text-sm font-bold text-black truncate">{userData?.name || "User"}</p>
                </div>
                {user.role === "admin" ? (
                  <>
                ...
                <button
                  ...
                  className="w-full text-left px-0 min-h-[44px] flex items-center text-sm text-red-500 font-bold mt-2 pt-3 border-t border-black/5"
                >
                  Logout
                </button>
              </div>
              ...
              <button
                ...
                className="w-full py-4 rounded-xl bg-black text-white text-xs font-bold uppercase tracking-widest text-center hover:bg-gray-800 transition-colors"
              >
                Login / Sign Up
              </button>
            )}
