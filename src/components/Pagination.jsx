...
export default function Pagination({
  ...
  const btn =
    "min-w-[34px] h-[34px] px-2 flex items-center justify-center rounded-lg text-sm font-semibold";
  const border = {
    border:
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-5">
      ...
      >
               {" "}
        <span>
          Showing <b>{from}</b>–<b>{to}</b> of <b>{total}</b> {itemLabel}
        </span>
               {" "}
        {onPageSizeChange && (
      ...
      <div className="flex items-center gap-1.5">
        ...
        {pages.map((p, i) =>
          p === "…" ? (
            ...
            <button
              ...
              style={
                p === page
                  ? { background: accent, color: "var(--color-inverse)" }
                  : border
              }
            >
                            {p}           {" "}
        ...
        <button
          className={btn}
          style={{ ...border, ...disabled(page === totalPages) }}
