...
const ProcessDetailsPage = lazy(
  () => import("./pages/ProcessDetailsPage/ProcessDetailsPage"),
);
...
const AddDiscountPage = lazy(
  () => import("./pages/AddDiscountPage/AddDiscountPage"),
);
const EditProductPage = lazy(
  () => import("./pages/EditProductPage/EditProductPage"),
);
const UserOrdersPage = lazy(
  () => import("./pages/UserOrdersPage/UserOrdersPage"),
);
const OrderDetailPage = lazy(
  () => import("./pages/OrderDetailPage/OrderDetailPage"),
);
const UserPastOrdersPage = lazy(
  () => import("./pages/UserPastOrdersPage/UserPastOrdersPage"),
);
const MyProfile = lazy(() => import("./pages/MyProfile/MyProfile"));
const AllUsersOrdersAnalytics = lazy(
  () => import("./pages/AllUsersOrdersAnalytics/AllUsersOrdersAnalytics"),
);
const AllProductsOrdersAnalytics = lazy(
  () => import("./pages/AllProductsOrdersAnalytics/AllProductsOrdersAnalytics"),
);
const AdminUploadOrders = lazy(
  () => import("./pages/AdminUploadOrders/AdminUploadOrders"),
);
const ScrollSequence = lazy(
  () => import("./pages/ScrollSequence/ScrollSequence"),
);
const CreateOrdersPage = lazy(
  () => import("./pages/CreateOrdersPage/CreateOrdersPage"),
);
...
const areCartItemsEqual = (arr1, arr2) => {
  ...
  for (const item1 of arr1) {
    ...
    if (Number(item1.discount || 0) !== Number(item2.discount || 0))
      return false;
  }
  return true;
};
...
const App = () => {
  ...
  const authUser = useSelector((state) => state.user.user); // Single-session enforcement: logs this device out if another device logs in.

  useSessionWatcher(authUser?.uid);

  const [authChecked, setAuthChecked] = useState(false); // Keep a ref to current cartItems to prevent infinite listener update loops

  const cartItemsRef = useRef(cartItems);
  useEffect(() => {
    cartItemsRef.current = cartItems;
  }, [cartItems]); // AUTH + LOAD USER + CART + WISHLIST
  ...
  }, [dispatch]); // AUTO SAVE CART TO FIRESTORE
    ...
    <BrowserRouter>
      ...
      <Suspense fallback={<Loader />}>
        ...
        <Routes>
          ...
                 {" "}
          <Route
            path="/admin/allUsersOrdersAnalytics"
            element={
              <AdminRoute>
                <AllUsersOrdersAnalytics />
              </AdminRoute>
            }
          />
                 {" "}
          <Route
            ...
            element={
              <AdminRoute>
                <AllProductsOrdersAnalytics />
              </AdminRoute>
            }
          />
                 {" "}
          <Route
            path="/admin/adminUploadOrders"
            element={
              <AdminRoute>
                <AdminUploadOrders />
              </AdminRoute>
