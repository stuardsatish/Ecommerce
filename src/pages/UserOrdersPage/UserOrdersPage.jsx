...
/* ---------------- FETCH USER ORDERS ---------------- */
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      setLoading(true)
      try {
        // Query by userId only (single-field, no composite index needed).
        // Ongoing status filtering and sorting are performed client-side so
        // a missing composite index can never silently return an empty list.
        const q = query(
          collection(fireDB, "orders"),
          where("userId", "==", user.uid)
        )
        const snap = await getDocs(q)
        const rawOrders = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }))
        ...
        // Filter: keep only ongoing orders (case-insensitive).
  ...
  useEffect(() => {
    const fetchUserReviews = async () => {
      ...
      try {
        ...
        const reviewedProductIds = snap.docs.map(d => d.data().productId).filter(Boolean)
...
/* ---------------- UI ---------------- */
  ...
  if (isMobile) {
    const statusStyles = {
      placed: { background: "#EDE7F6", color: "#5B3FA8" },
      confirmed: { background: "#E3F2FD", color: "#1565C0" },
      shipped: { background: "#FFF3E0", color: "#E65100" },
      delivered: { background: "#E8F5E9", color: "#A43B31" },
    }
    return (
        ...
        <main style={{ padding: "96px 20px 24px" }}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center text-center" style={{ minHeight: "60vh", gap: "16px" }}>
              <Package size={44} color="#C4C6CD" />
              <p style={{ color: "#74777D", fontSize: "14px", fontWeight: 600 }}>No active orders yet.</p>
            ...
            <div className="flex flex-col" style={{ gap: "16px" }}>
              {orders.map((order) => {
                const badge = statusStyles[order.orderStatus] || { background: "#F0EEED", color: "#44474C" }
                return (
                        ...
                        <div key={index} className="flex items-center justify-between" style={{ gap: "10px" }}>
                          ...
                          {order.orderStatus === "delivered" && (
                            userReviews.includes(item.productId) ? (
                              <span className="flex items-center flex-shrink-0" style={{ gap: "4px", fontSize: "10px", fontWeight: 600, color: "#A43B31" }}>
                                <CheckCircle2 size={12} /> Reviewed
                              </span>
                            ) : (
                              <button onClick={() => openReviewModal(item, order.id)} className="flex items-center flex-shrink-0" style={{ gap: "4px", fontSize: "11px", fontWeight: 700, color: "#A43B31" }}>
                                <Star size={12} /> Review
                              </button>
                            )
                          )}
    ...
    <div ref={containerRef} className="hidden lg:block" style={{ background: "#FBF9F8", minHeight: "100vh", fontFamily: "Inter, sans-serif", color: "#1B1C1C" }}>
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "48px 40px 80px" }}>
        ...
        <div className="flex" style={{ borderBottom: "1px solid #E4E2E1", gap: "16px", paddingBottom: "4px", marginTop: "24px" }}>
          <span style={{ padding: "8px 16px", fontWeight: 600, fontSize: "12px", letterSpacing: "0.6px", color: "#A43B31", borderBottom: "2px solid #A43B31", marginBottom: "-5px" }}>
            Ongoing Orders
          </span>
        </div>
        ...
        <div className="flex flex-col" style={{ gap: "24px", marginTop: "24px" }}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center text-center" style={{ gap: "16px", padding: "80px 0", border: "1px dashed #E4E2E1", borderRadius: "8px", background: "#fff" }}>
              ...
              <p style={{ fontWeight: 700, fontSize: "18px", color: "#1B1C1C" }}>No ongoing orders</p>
              <p style={{ fontSize: "14px", color: "#44474C" }}>You have no active orders at the moment.</p>
