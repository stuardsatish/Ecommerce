...
const UserOrdersPage = () => {
  ...
  const navigate = useNavigate(); // Mobile design renders below `lg`; the desktop redesign at lg+.
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const ongoingOrders = enrichedOrders
          .filter((o) =>
            ONGOING_STATUSES.has((o.orderStatus || "").toLowerCase()),
          )
          .sort((a, b) => {
  ...
  useEffect(() => {
    const fetchUserReviews = async () => {
      ...
      try {
        ...
        const snap = await getDocs(q); // Track reviewed products (server allows one review per product per user).
          ...
          {loading ? (
            ...
            <div className="flex flex-col" style={{ gap: "16px" }}>
              ...
              {pagedOrders.map((order) => {
                const badge = statusStyles[order.orderStatus] || {
                        ...
                        <button
                          onClick={() =>
                            generateInvoice(order, {
                              name: order.userName || userData?.name,
                              email: order.userEmail || userData?.email,
                              phone: order.userPhone || userData?.phone,
                  ...
                  return (
                    ...
                    >
                      ...
                      <Star
                        size={30}
                        style={{
                          fill: on ? "var(--color-chart-gold)" : "none",
                          color: on
  ...
  return (
    ...
    >
      ...
      >
                {/* SECTION 1 — HEADER */}       {" "}
        <div
          className="header-text flex flex-col"
          style={{ gap: "4px", paddingBottom: "8px" }}
        >
                   {" "}
          <h1
            style={{
          ...
          >
                        Welcome back, {userData?.name || user?.name || "there"}.
            Here's a summary of your recent purchases and their status.        
             {" "}
          </p>
                 {" "}
        </div>
