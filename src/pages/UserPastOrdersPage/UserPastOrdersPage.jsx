...
const UserPastOrdersPage = () => {
  ...
  const [loading, setLoading] = useState(true); // pagination (shared by mobile list + desktop tabs)
...
  ============================================================ */ /* ---------------- DESKTOP TABS + HANDLERS ---------------- */
  ...
  const desktopOrders =
    allOrders.filter(
      tabFilter,
    ); /* ---------------- PAGINATION ---------------- */ // Reset to first page whenever the active tab changes.

  useEffect(() => {
    setPage(1);
  }, [activeTab]); // Mobile shows delivered-only (`orders`); desktop shows the tab-filtered list.
  const mobileTotal = orders.length;
  const desktopTotal = desktopOrders.length;
  const pagedMobileOrders = orders.slice(
    (page - 1) * pageSize,
    page * pageSize,
  );
  const pagedDesktopOrders = desktopOrders.slice(
    (page - 1) * pageSize,
    page * pageSize,
  ); // Clamp the page if the current total shrinks below its range.
  useEffect(() => {
                            ...
                            >
                              ...
                              {[1, 2, 3, 4, 5].map((star) => (
                                <Star
                                  ...
                                  style={{
                                    fill:
                                      userReviews[item.productId].rating >= star
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Your order history — all past purchases.
          </p>
                 {" "}
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontSize: "12px",
                  letterSpacing: "0.6px",
                  color: act ? "var(--color-primary)" : "var(--color-body)",
                  borderBottom: act
