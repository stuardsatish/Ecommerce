...
  // pagination (shared by mobile list + desktop tabs)
...
  // Mobile shows delivered-only (`orders`); desktop shows the tab-filtered list.
...
                              {[1, 2, 3, 4, 5].map((star) => (
                                <Star key={star} size={10} style={{ fill: userReviews[item.productId].rating >= star ? "var(--color-chart-gold)" : "none", color: userReviews[item.productId].rating >= star ? "var(--color-chart-gold)" : "var(--color-border-strong)" }} />
                              ))}
                            </div>
