...
const CartPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (active) {
          if (paySnap.exists()) {
            const pd = paySnap.data(); // COD defaults to enabled for docs created before the flag existed.
          ...
          if (shipSnap.exists()) {
            ...
            setShippingConfig({
              freeShippingThreshold:
                typeof sd.freeShippingThreshold === "number"
                  ? sd.freeShippingThreshold
                  : 500,
              shippingCost:
                typeof sd.shippingCost === "number" ? sd.shippingCost : 49,
            });
          }
        }
      } catch (error) {
  ...
  const isCheckoutDisabled =
    ...
    !paymentSettings.codPayment; // Mobile design renders below the `lg` breakpoint; the desktop redesign at lg+.

  const isMobile =
    useIsMobile(1024); /* ---------- DESKTOP REDESIGN STATE ---------- */
  ...
  const money = (n) => `₹${Number(n || 0).toFixed(2)}`; // const subtotal = cartItems.reduce((s, i) => s + Number(i.price) * i.quantity, 0)
  ...
  }, 0); // Calculate promo discount dynamically
  ...
  const shippingEstimate =
    subtotal >= shippingConfig.freeShippingThreshold
      ? 0
      : shippingConfig.shippingCost; //const estimatedTax = subtotal * 0.08
  // const orderTotal = Math.max(0, subtotal + shippingEstimate + estimatedTax - promoDiscount)
  const orderTotal = Math.max(
    0,
    subtotal + shippingEstimate - promoDiscount,
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      ...
      toast.success(
        data.discount > 0
          ? `Promo applied: -${money(data.discount)}`
          : "Promo applied",
      );
    } catch (e) {
  ...
  const handleDeleteItem = (id) => dispatch(deleteCart(id)); /* ADD CART */
...
     order/stock/analytics docs (those are admin-only at the rules level). */
  ...
  const placeOrder = async () => {
    try {
      ...
      setIsProcessing(true); /* Prefill info only — NOT prices. */
          ...
          {paymentSettings.razorpayPayment && (
            <button
              ...
              style={{
                ...
                boxShadow:
                  "0px 4px 12px color-mix(in srgb, var(--color-primary) 20%, transparent)",
                fontWeight: 700,
                fontSize: "15px",
                cursor: "pointer",
                border: "none",
  ...
  if (isMobile) {
      ...
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 backdrop-blur-2xl bg-overlay">
        ...
        <div className="bg-surface/10 backdrop-blur-md border border-inverse/20 rounded-[2rem] p-10 max-w-sm w-full text-center shadow-2xl">
          ...
          <div className="relative mb-8 h-20 flex items-center justify-center">
            ...
            <Package
              size={56}
              className="text-inverse relative animate-bounce"
            />
                     {" "}
          </div>
                   {" "}
    ...
    return (
      ...
      >
        ...
                {processingOverlay}        {paymentModal}     {" "}
      </div>
    );
  }
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: "1px solid var(--color-border)",
                        boxShadow:
                          "0px 4px 12px color-mix(in srgb, var(--color-accent) 5%, transparent)",
                        borderRadius: "12px",
                        padding: "24px",
                        gap: "24px",
                      }}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    assist with your                     shopping experience.  
                                   {" "}
                  </p>
                                   {" "}
                  <button
