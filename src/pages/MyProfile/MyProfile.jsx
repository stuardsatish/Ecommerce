...
        style={{ width: size, height: size, borderRadius: "9999px", background: "#FFDAD5" }}
      >
        {photoURL ? (
...
        style={{ bottom: 0, right: 0, width: "24px", height: "24px", borderRadius: "9999px", background: C.primary, border: "2px solid #FFFFFF" }}
      >
...
        background: emailVerified ? "#D1FAE5" : "#FFDAD5",
        color: emailVerified ? C.green : "#84241C",
      }}
    >
...
  const skeletonBlock = (style) => <div className="profile-skeleton" style={{ background: C.inputBg, borderRadius: "8px", ...style }} />
  const renderSkeleton = () => (
    <div className="flex flex-col items-center" style={{ gap: "16px", padding: isMobile ? "24px 20px" : "40px 24px", maxWidth: isMobile ? "100%" : "860px", margin: "0 auto", width: "100%" }}>
      {skeletonBlock({ width: isMobile ? "80px" : "96px", height: isMobile ? "80px" : "96px", borderRadius: "9999px" })}
      {skeletonBlock({ width: "180px", height: "20px" })}
      {skeletonBlock({ width: "140px", height: "14px" })}
      <div className="w-full" style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "16px" }}>
        {[...Array(5)].map((_, i) => (
          <div key={i} className="profile-skeleton" style={{ background: C.inputBg, borderRadius: "8px", width: "100%", height: "44px" }} />
        ))}
      </div>
    </div>
  )
...
            style={{ background: C.primary, borderRadius: "9999px", padding: "10px 24px", fontSize: "14px", fontWeight: 700, color: "#fff", boxShadow: "0px 4px 6px -4px rgba(164,59,49,0.2)", gap: "8px", minWidth: "140px", opacity: saving ? 0.85 : 1 }}
          >
...
        background: C.textPri, color: "#fff", borderRadius: "12px",
...
      .profile-input:focus{border-color:${C.textPri};background:#FFFFFF;}
      .profile-input::placeholder{color:${C.textSec};}
      @keyframes myprofileSpin{to{transform:rotate(360deg);}}
      .myprofile-spinner{width:16px;height:16px;border:2px solid rgba(255,255,255,0.4);border-top-color:#fff;border-radius:9999px;animation:myprofileSpin .7s linear infinite;}
    `}</style>
  )
...
          style={{ display: "none", height: "64px", background: "rgba(251,249,248,0.8)", backdropFilter: "blur(6px)", WebkitBackdropFilter: "blur(6px)", borderRadius: "12px 12px 0 0", boxShadow: "0px 10px 15px -3px rgba(0,0,0,0.1), 0px 4px 6px -4px rgba(0,0,0,0.1)" }}
        >
          {[
...
            const Icon = n.icon
            return (
              <button key={n.label} onClick={() => !n.active && navigate(n.to)} className="flex flex-col items-center justify-center" style={{ gap: "4px", color: n.active ? C.primary : "#44474C" }}>
                <Icon size={20} />
