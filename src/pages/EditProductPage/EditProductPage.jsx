...
const EditProductPage = () => {
    ...
    const saveProduct = async () => {
        ...
        if (productType === "single" && !form.price) return alert("Price is required for single products")
        ...
        try {
            ...
            const savedPrice = isVariant
                ? Math.min(...variantsSaved.map((v) => v.price))
                : Number(form.price) || 0
            const savedStock = isVariant
                ? variantsSaved.reduce((s, v) => s + v.stock, 0)
                : Number(form.stock) || 0
                            ...
                            <div className="relative">
                                ...
                                {productType === "single" ? (
                                    ...
                                    <input type="text" readOnly
                                        ...
                                        className={`${inputCls} pl-7`} style={{ opacity: 0.6, cursor: "not-allowed" }} />
                                )}
                            </div>
                        </div>
