...
const AdminOrdersPage = () => {
  ...
  const markAsPaid = async (orderId) => {
    try {
      await updateDoc(doc(fireDB, "orders", orderId), { paymentStatus: "paid" })
    ...
    return (
      ...
      >
        ...
        */}
        ...
        {column === "confirmed" && (
          <button onClick={(e) => { e.stopPropagation(); updateStatus(order.id, "shipped") }} className="w-full text-inverse" style={{ background: "var(--color-primary)", borderRadius: "12px", height: "52px", boxShadow: "0px 4px 6px rgba(0,0,0,0.1)", fontFamily: INTER, fontWeight: 700, fontSize: "16px" }}>Initiate Transit</button>
        )}
        {column === "shipped" && (
          <div className="flex flex-col gap-3">
            ...
            <button onClick={(e) => { e.stopPropagation(); updateStatus(order.id, "delivered") }} className="w-full" style={{ background: "var(--color-accent-subtle)", color: "var(--color-body)", borderRadius: "12px", height: "44px", fontFamily: INTER, fontWeight: 700, fontSize: "14px" }}>Mark Delivered</button>
          </div>
        )}
      </div>
    )
