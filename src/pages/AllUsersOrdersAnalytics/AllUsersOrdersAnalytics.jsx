...
const rankBadgeStyle = (rank) => {
  if (rank === 1)
    return {
      background:
        "linear-gradient(135deg,var(--color-chart-gold),color-mix(in srgb, var(--color-chart-gold) 55%, white))",
      color: "var(--color-ink)",
    };
  if (rank === 2)
    return {
      background:
        "linear-gradient(135deg,var(--color-chart-silver),color-mix(in srgb, var(--color-chart-silver) 55%, white))",
      color: "var(--color-ink)",
    };
  if (rank === 3)
    return {
      background:
        "linear-gradient(135deg,var(--color-chart-bronze),color-mix(in srgb, var(--color-chart-bronze) 55%, white))",
      color: "var(--color-ink)",
    };
  return null;
};
...
const AllUsersOrdersAnalytics = () => {
  ...
  const PAGE_SIZE = 10; // Order detail drawer — `displayedOrder` keeps the last order's data visible
  ...
  const fetchUsersOrders = async () => {
    try {
      setLoading(true); // 1. Users → metadata buckets keyed by uid
      ...
      ordersSnap.docs.forEach((o) => {
        ...
        usersById[uid].orders.push({
          ...
          address:
            data.userAddress || data.address || data.shippingAddress || "",
        });
      });
  ...
  const metrics = useMemo(() => {
    const now = new Date();
    const periodDays = PERIOD_DAYS[filter] ?? 30; // Custom range: only active once both From/To are picked and From <= To;
    ...
    const last30 = new Date(now.getTime() - 30 * 86400000); // Per-user stats within the active period

    const users = rawUsers.map((u) => {
      const inPeriod = u.orders.filter(
        (o) => o.date && o.date >= periodStart && o.date <= periodEnd,
      );
      const allTimeTotal = u.orders.reduce((s, o) => s + o.total, 0);
      const periodTotal = inPeriod.reduce((s, o) => s + o.total, 0);
      const last30Count = u.orders.filter(
        (o) => o.date && o.date >= last30,
      ).length;
      const lastOrderDate = u.orders.reduce(
        (m, o) => (o.date && (!m || o.date > m) ? o.date : m),
        null,
      );
      const total = periodTotal; // Orders shown in the expandable detail list — scoped to the active range
    ...
    const activeCustomers = users.filter((u) => u.last30Count >= 1).length; // Period-over-period trends (from all orders)
    ...
    rawUsers.forEach((u) => {
      if (
        u.orders.some((o) => o.date && o.date >= prev30Start && o.date < last30)
      )
        prevActive += 1;
    });
    const trends = {
      ...
      aov: trendOf(
        curOrd ? curRev / curOrd : 0,
        prevOrd ? prevRev / prevOrd : 0,
      ),
    ...
    rawUsers.forEach((u) =>
      u.orders.forEach((o) => {
        ...
        if (o.date >= within) {
          const key = o.date.toLocaleDateString("en-IN", {
            day: "2-digit",
            month: "short",
          });
    ...
    const dailyAverage = seriesVals.length
      ? seriesVals.reduce((a, b) => a + b, 0) / seriesVals.length
      : 0;
    ...
    for (let i = 1; i < seriesVals.length; i++) {
      if (seriesVals[i - 1] > 0) {
        const g =
          ((seriesVals[i] - seriesVals[i - 1]) / seriesVals[i - 1]) * 100;
    ...
    const ordersToday = rawUsers.reduce(
      (s, u) =>
        s +
        u.orders.filter(
          (o) => o.date && o.date >= new Date(now.getTime() - 86400000),
        ).length,
      0,
    );
    const currentVelocity = ordersToday / 24; // Customer segments
  ...
  }, [rawUsers, filter, search, minRev, maxRev, sortBy, customFrom, customTo]); // Jump both paginated sections back to page 1 whenever the active filters change,
  ...
  const topSpendingTotalPages = Math.max(
    1,
    Math.ceil(metrics.displayed.length / PAGE_SIZE),
  );
  const topSpendingSafePage = Math.min(topSpendingPage, topSpendingTotalPages);
  const topSpendingPageItems = metrics.displayed.slice(
    (topSpendingSafePage - 1) * PAGE_SIZE,
    topSpendingSafePage * PAGE_SIZE,
  );

  const detailedOverviewTotalPages = Math.max(
    1,
    Math.ceil(metrics.displayed.length / PAGE_SIZE),
  );
  const detailedOverviewSafePage = Math.min(
    detailedOverviewPage,
    detailedOverviewTotalPages,
  );
  const detailedOverviewPageItems = metrics.displayed.slice(
    (detailedOverviewSafePage - 1) * PAGE_SIZE,
    detailedOverviewSafePage * PAGE_SIZE,
  ); /* ---------- ANIMATIONS ---------- */
  ...
  const exportToExcel = () => {
    ...
    const summary = [
      ...
      [
        "Top Spender",
        m.topSpender
          ? `${m.topSpender.name} (INR ${Math.round(m.topSpender.total)})`
          : "—",
      ],
    ];
    const customers = m.displayed.map((u, i) => ({
      ...
      "Last Order": u.lastOrderDate
        ? u.lastOrderDate.toLocaleDateString("en-IN")
        : "—",
    }));
    const trend = (m.revenueSeries || []).map((d) => ({
      Date: d.date,
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.json_to_sheet(trend),
      "Revenue Trend",
    );
    XLSX.writeFile(
      wb,
      `Users_Analytics_${periodLabel.replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
...
    ============================================================ */
          ...
          <div className="px-8 py-6">
            ...
            <div className="mb-6 flex items-start justify-between gap-4">
              ...
              <div className="flex flex-col gap-2">
                {bar({ width: "320px", height: "24px" })}
                {bar({ width: "260px", height: "14px" })}
              </div>
                                         {" "}
            ...
                                    {/* Filter panel */}                       {" "}
            <div
              className="flex flex-wrap items-center gap-3 p-4 mb-6 rounded-xl bg-surface"
              style={{ border: `1px solid ${C.border}` }}
            >
                                         {" "}
              {[...Array(4)].map((_, i) => (
              ...
              <div className="flex-1 min-w-[200px]">
                {bar({ width: "100%", height: "36px", borderRadius: "8px" })}
              </div>
                                         {" "}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                                         {" "}
              <div
                className="lg:col-span-2 rounded-xl bg-surface p-6"
                style={{ border: `1px solid ${C.border}` }}
              >
                                               {" "}
                <div className="flex items-center justify-between mb-4">
                  {bar({ width: "140px", height: "18px" })}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
              ...
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="rounded-xl bg-surface p-4 flex items-center gap-3"
                  style={{ border: `1px solid ${C.border}` }}
                >
                                                     {" "}
                  {bar({
            ...
            >
              ...
              <div
                ...
                style={{
                  background: "var(--color-surface-muted)",
                  borderBottom: `2px solid ${C.darkDesk}`,
                }}
              >
                                               {" "}
                {[...Array(7)].map((_, i) => (
          ...
          <div className="px-4 pt-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              ...
              {[...Array(6)].map((_, i) => (
                ...
                >
                                                     {" "}
                  {bar({ width: "70%", height: "11px", marginBottom: "8px" })}
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                {bar({ width: "120px", height: "16px" })}
                {bar({ width: "50px", height: "16px" })}
              </div>
                                         {" "}
            ...
            <div className="flex gap-3 overflow-x-auto no-scrollbar mb-5 pb-1">
              ...
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="flex-shrink-0 rounded-2xl bg-surface p-4 flex items-center gap-3"
                  style={{ width: "230px", border: `1px solid ${C.border}` }}
                >
                                                     {" "}
                  {bar({
                    width: "44px",
  ...
  const KpiTrend = ({ t, label }) => (
    <span
      ...
      style={{ color: t.up ? C.green : C.red }}
    >
      ...
      <div className="hidden md:block">
        ...
        <div className="px-8 py-6">
          ...
          <div className="mb-6 flex items-start justify-between gap-4">
            ...
            <div>
                                         {" "}
              <h1 className="text-2xl font-bold" style={{ color: C.darkDesk }}>
                All Users Orders Analytics
              </h1>
                                         {" "}
              <p className="text-sm" style={{ color: C.textSec }}>
                Real-time performance metrics and customer lifecycle data
              </p>
                                     {" "}
            </div>
                                   {" "}
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-inverse text-sm font-semibold flex-shrink-0"
              style={{ background: C.darkDesk }}
            >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
            ...
            <KpiCard
              ...
              trend={<KpiTrend t={m.trends.users} label="vs prev" />}
            />
                                   {" "}
            <KpiCard
              className="kpi-card"
              ...
              trend={<KpiTrend t={m.trends.orders} label="" />}
            />
                                   {" "}
            <KpiCard
              className="kpi-card"
              ...
              value={formatCompactINR(m.totalRevenue)}
              trend={<KpiTrend t={m.trends.revenue} label="" />}
            />
                                   {" "}
            <KpiCard
              className="kpi-card"
              ...
              value={formatINR(Math.round(m.aov))}
              trend={<KpiTrend t={m.trends.aov} label="" />}
            />
                                   {" "}
            <KpiCard
              className="kpi-card"
              ...
              trend={
                <span className="text-xs" style={{ color: C.textSec }}>
                  Spent {formatINR(m.topSpender?.total || 0)}
                </span>
              }
            />
                                   {" "}
            <KpiCard
              ...
              label="Active Users"
              icon={<Zap size={16} />}
              value={formatCount(m.activeCustomers)}
              trend={<KpiTrend t={m.trends.active} label="Retention" />}
            />
                               {" "}
          </div>
                              {/* FILTER PANEL */}                   {" "}
          <div
            className="sticky top-[57px] z-20 flex flex-wrap items-center gap-3 p-4 mb-6 rounded-xl bg-surface"
            style={{ border: `1px solid ${C.border}` }}
          >
                                   {" "}
            ...
            {FILTERS.map((f) => (
              <button
                ...
                style={
                  filter === f.key
                    ? { background: C.darkDesk, color: "var(--color-inverse)" }
                    : {
                        background: "var(--color-surface-muted)",
                        color: C.textSec,
                      }
            ...
            {filter === "custom" && (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="outline-none rounded-lg text-sm px-2 py-1.5"
                  style={{ border: `1px solid ${C.border}` }}
                />
                                               {" "}
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full outline-none rounded-lg text-sm"
                style={{
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                type="number"
                placeholder="Min"
                className="w-20 outline-none rounded-lg text-sm px-2 py-1.5"
                style={{ border: `1px solid ${C.border}` }}
              />
                                         {" "}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                                               {" "}
                <h3 className="font-bold" style={{ color: C.darkDesk }}>
                  Revenue Trend
                </h3>
                                               {" "}
                <div
              ...
              >
                ...
                <SummaryStat
                  label="Daily Average"
                  value={formatINR(Math.round(m.dailyAverage))}
                />
                                               {" "}
                <SummaryStat
                  label="Peak Growth"
                  value={`+${Math.round(m.peakGrowth)}%`}
                  green
                />
                                               {" "}
                <SummaryStat
                  label="Current Velocity"
                  value={`${m.currentVelocity.toFixed(1)} ord/hr`}
                />
                                           {" "}
              </div>
                                     {" "}
            ...
            >
                                         {" "}
              <h3 className="font-bold mb-4" style={{ color: C.darkDesk }}>
                Customer Segments
              </h3>
                                         {" "}
              <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span
                    ...
                    style={{ color: C.darkDesk }}
                  >
                    {formatCount(m.totalUsers)}
                  </span>
                                                     {" "}
                  <span className="text-xs" style={{ color: C.textSec }}>
                    Total Users
                ...
                {m.segments.map((s, i) => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ color: C.darkDesk }}
                    >
                      {segTotal ? Math.round((s.value / segTotal) * 100) : 0}%
                    </span>
                                                       {" "}
                  </div>
                ))}
          ...
          <div className="flex items-center justify-between mb-3">
                                   {" "}
            <h2 className="text-lg font-bold" style={{ color: C.darkDesk }}>
              Customer Highlights
            </h2>
                                   {" "}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
            ...
            {m.displayed.slice(0, 6).map((u, i) => (
              <div
                key={u.userId}
                className="insight-card rounded-xl bg-surface p-4 flex items-center gap-3"
                style={{ border: `1px solid ${C.border}` }}
              >
                                               {" "}
                <div className="relative flex-shrink-0">
                  ...
                                       {" "}
                  <span
                    className="absolute -top-1 -right-1 text-[10px] font-bold text-inverse rounded-full px-1.5"
            ...
            {m.displayed.length === 0 && (
              <p className="text-sm col-span-full" style={{ color: C.textSec }}>
                No customers yet.
              </p>
            )}
                               {" "}
          ...
                              {/* TOP SPENDING TABLE */}                   {" "}
          <h2 className="text-lg font-bold mb-3" style={{ color: C.darkDesk }}>
              ...
              <thead>
                ...
                >
                  ...
                  {[
                    ...
                    <th
                      key={h}
                      className="text-left font-semibold px-4 py-3 text-xs tracking-wide"
                      style={{ color: C.textSec }}
                    >
                      {h}
                    </th>
                ...
                {topSpendingPageItems.map((u, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                                                                 {" "}
                      <td className="px-4 py-3" style={{ color: C.textSec }}>
                        {formatINR(Math.round(u.aov))}
                      </td>
                                                                 {" "}
                      <td className="px-4 py-3" style={{ color: C.textSec }}>
                        {relativeTime(u.lastOrderDate)}
                      </td>
                                                             {" "}
                    </tr>
                  );
            ...
            {detailedOverviewPageItems.map((u) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Avatar
                    user={u}
                    highlight={u.userId === m.displayed[0]?.userId}
                  />
                                                     {" "}
                  <div className="flex-1 min-w-0">
                                                           {" "}
                    <p className="font-bold" style={{ color: C.darkDesk }}>
                      {u.name}
                    </p>
                                                           {" "}
                    <p
                  ...
                  >
                    ...
                    {u.periodOrders.length === 0 ? (
                      ...
                      u.periodOrders.map((o) => (
                        <div
                          key={o.id}
                          onClick={() => openOrder(u, o)}
                          className="flex items-center justify-between text-sm py-2 px-2 -mx-2 rounded-lg cursor-pointer hover:bg-[var(--color-surface-muted)] transition-colors"
                        >
                                                                             {" "}
                          <span
                            style={{
                          ...
                          <span style={{ color: C.textSec }}>
                            {o.date
                              ? o.date.toLocaleDateString("en-IN", {
                                  day: "numeric",
                                  month: "short",
                                  year: "numeric",
                                })
...
      ============================================================ */}
      ...
      <div className="md:hidden pb-24" style={{ background: C.bg }}>
        ...
        <div className="px-4 pt-4">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4">
            ...
            {FILTERS.map((f) => (
              <button
                ...
                style={
                  filter === f.key
                    ? { background: C.blue, color: "var(--color-inverse)" }
                    : {
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            ...
            <div className="rounded-xl p-4" style={{ background: C.blueMob }}>
              ...
                             {" "}
              <p className="font-bold text-inverse mt-1 truncate">
                {m.topSpender?.name || "—"}
              </p>
                                         {" "}
              <p className="font-bold text-inverse text-lg">
                {formatINR(m.topSpender?.total || 0)}
              </p>
                                     {" "}
            </div>
                               {" "}
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div>
                                               {" "}
                <h3 className="font-bold" style={{ color: C.darkMob }}>
                  Revenue Trend
                </h3>
                                               {" "}
                <p className="text-xs" style={{ color: C.textSec }}>
              ...
              <div className="text-right">
                ...
                <p
                  className="font-bold"
                  style={{ color: m.trends.revenue.up ? C.green : C.red }}
                >
                                                     {" "}
                  {m.trends.revenue.up ? "+" : "-"}
                  {m.trends.revenue.pct}%                                {" "}
                </p>
                                               {" "}
                <p
            ...
            <ResponsiveContainer width="100%" height={150}>
              ...
              <BarChart data={m.revenueSeries.slice(-7)}>
                ...
                <Tooltip
                  ...
                  cursor={{
                    fill: "color-mix(in srgb, var(--color-chart-3) 5%, transparent)",
                  }}
                />
                                               {" "}
            ...
            >
              ...
              <div>
                ...
                <p className="font-bold" style={{ color: C.darkMob }}>
                  {formatINR(Math.round(m.todayRevenue))}
                </p>
              </div>
                                         {" "}
              <div className="text-right">
                ...
                <p className="font-bold" style={{ color: C.darkMob }}>
                  {formatCompactINR(m.totalRevenue)}
                </p>
              </div>
                                     {" "}
            </div>
            ...
            {m.displayed.slice(0, 6).map((u, i) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                                                     {" "}
                  <p className="font-bold text-sm" style={{ color: C.blue }}>
                    {formatINR(u.total)}
                  </p>
                                                 {" "}
                </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
                       {" "}
              <span
                style={{ fontFamily: MONO, fontSize: "12px", color: C.textSec }}
              >
                                                /users/
                <span style={{ color: C.blue }}>userId</span>/orders            
                               {" "}
              </span>
                                     {" "}
            </div>
            ...
            <div className="flex items-start gap-3 mb-2">
              ...
              <span
                ...
                style={{ background: C.blue }}
              >
                <UserIcon size={14} color="var(--color-inverse)" />
              </span>
                                         {" "}
              <div
            ...
            <div className="flex items-start gap-3">
              ...
              >
                <ShoppingBag size={14} color={C.textSec} />
              </span>
                                         {" "}
              <div
                className="flex-1 rounded-xl p-3"
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <span
              ...
              style={{ background: "var(--color-border)", color: C.textSec }}
            >
          ...
                             {" "}
          <div
            className="rounded-2xl bg-surface overflow-hidden mb-4"
            style={{ border: `1px solid ${C.border}` }}
          >
                                   {" "}
            {(showAllLeaderboard ? m.displayed : m.displayed.slice(0, 5)).map(
              (u, i) => (
                <div
                  key={u.userId}
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: C.blue, opacity: 0.4 }}
                  >
        ...
        <button
          ...
          className="md:hidden fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full flex items-center justify-center shadow-lg"
          style={{ background: C.blue }}
        >
                             {" "}
          ...
          <>
            ...
            >
              ...
              <div>
                ...
                >
                                                      #
                  {displayedOrder.externalOrderId
                    ? String(displayedOrder.externalOrderId).replace(/^#/, "")
                    : displayedOrder.id}
                                                 {" "}
                </p>
                                           {" "}
              ...
              <div className="text-right">
                ...
                <p
                  ...
                  style={{ color: C.darkDesk }}
                >
                  {formatINR(displayedOrder.total)}
                </p>
                                           {" "}
              </div>
                                     {" "}
            ...
            <div className="flex items-center justify-between">
              ...
              >
                                               {" "}
                {displayedOrder.date
                  ? displayedOrder.date.toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })
            ...
            <div>
              ...
              >
                ...
                <span
                  ...
                  style={{ color: C.darkDesk }}
                >
                  {displayedOrder.customerName ||
                    displayedOrder.userName ||
                    "Guest"}
                </span>
                                               {" "}
                <span
                  className="text-xs"
                  style={{ fontFamily: MONO, color: C.textSec }}
                >
                  {displayedOrder.customerEmail ||
                    displayedOrder.userEmail ||
                    "—"}
                </span>
                                               {" "}
                {displayedOrder.userPhone && (
                  <span className="text-xs" style={{ color: C.textSec }}>
                    {displayedOrder.userPhone}
                  </span>
                )}
            ...
            <div>
              ...
              >
                ...
                <span className="text-sm" style={{ color: C.textSec }}>
                  Payment:{" "}
                  <span style={{ color: C.darkDesk, fontWeight: 600 }}>
                ...
                <span className="text-sm" style={{ color: C.textSec }}>
                  Shipping Address:{" "}
                  <span style={{ color: C.darkDesk, fontWeight: 600 }}>
                    {displayedOrder.address || "No address on file"}
                  </span>
                </span>
                                           {" "}
              </div>
...
const KpiCard = ({ label, icon, value, trend, className }) => (
  <div
    className={`${className} rounded-xl bg-surface p-4`}
    style={{ border: `1px solid ${C.border}` }}
  >
           {" "}
    ...
           {" "}
    <div className="text-2xl font-bold mb-1" style={{ color: C.darkDesk }}>
      {value}
    </div>
            {trend}   {" "}
  </div>
...
const SummaryStat = ({ label, value, green }) => (
  <div className="text-center">
    ...
           {" "}
    <p className="font-bold" style={{ color: green ? C.green : C.darkDesk }}>
      {value}
    </p>
       {" "}
  </div>
...
const Stat = ({ label, value, green }) => (
  <div className="text-center min-w-[90px] hidden lg:block">
           {" "}
    <p className="text-[10px] tracking-wide" style={{ color: C.textSec }}>
      {label}
    </p>
           {" "}
    <p
      className="font-semibold text-sm"
      style={{ color: green ? C.green : C.darkDesk }}
    >
      {value}
    </p>
       {" "}
  ...
  if (user.photoURL) {
    return (
      <img
        ...
        style={{ ...s, border: highlight ? `2px solid ${C.green}` : "none" }}
      />
    );
  }
  return (
    <span
      className="rounded-full flex items-center justify-center flex-shrink-0 font-bold text-inverse"
      style={{
        ...s,
        background: avatarColor(user.name),
        border: highlight ? `2px solid ${C.green}` : "none",
        fontSize: size * 0.36,
      }}
    >
                  {initials(user.name)}       {" "}
...
const MobKpi = ({ label, value, trend, stable }) => (
  <div
    className="kpi-card rounded-xl bg-surface p-4"
    style={{ border: `1px solid ${C.border}` }}
  >
           {" "}
    <p className="text-xs" style={{ color: C.textSec }}>
    ...
    <div className="flex items-end gap-1.5 mt-1">
                 {" "}
      <span className="text-xl font-bold" style={{ color: C.darkMob }}>
        {value}
      </span>
                 {" "}
      {stable ? (
