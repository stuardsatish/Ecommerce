...
    AlertTriangle, Layers, FileDown, CheckCircle2, XCircle, ChevronRight, ArrowUp, ArrowDown,
} from "lucide-react"
import {
...
    electronics: { bg: "var(--color-accent-strong)", icon: "var(--color-inverse)", Icon: Cpu },
    fashion: { bg: "var(--color-info-subtle)", icon: "var(--color-body)", Icon: Shirt },
    apparel: { bg: "var(--color-info-subtle)", icon: "var(--color-body)", Icon: Shirt },
    furniture: { bg: "var(--color-info-subtle)", icon: "var(--color-ink)", Icon: Sofa },
    groceries: { bg: "var(--color-body)", icon: "var(--color-inverse)", Icon: Apple },
}
const catStyle = (name) => CATEGORY_STYLE[String(name || "").toLowerCase()] || { bg: "var(--color-surface-muted)", icon: C.indigo, Icon: Package }
const fileSafe = (s) => String(s || "").replace(/[^a-z0-9_-]+/gi, "_").replace(/^_+|_+$/g, "") || "NA"
const LOW_STOCK = 10
const stockStatus = (s) => (s <= 0 ? "out" : s <= LOW_STOCK ? "low" : "in")
const statusLabelOf = (s) => (stockStatus(s) === "out" ? "Out of Stock" : stockStatus(s) === "low" ? "Low Stock" : "In Stock")
...
    p.image || p.productImage || p.imageUrl || p.thumbnail || (Array.isArray(p.gallery) ? p.gallery[0] : "") || ""
...
        ) : (
            <span className="w-full h-full flex items-center justify-center" style={{ background: C.lightIndigo }}>
                <Package size={28} style={{ color: C.textS }} />
            </span>
        )
    }
    return src ? (
        <img src={src} alt={alt || ""} className="object-cover flex-shrink-0" style={{ width: size, height: size, border: `1px solid ${C.border}`, borderRadius: radius }} />
    ) : (
        <span className="flex items-center justify-center flex-shrink-0" style={{ width: size, height: size, background: C.lightIndigo, border: `1px solid ${C.border}`, borderRadius: radius }}>
            <Package size={Math.max(14, Math.round(size * 0.45))} style={{ color: C.textS }} />
        </span>
    )
}
...
    // Per-product analytics drawer (Analytics tab "View Analytics" button).
...
    const [displayedProductAnalytics, setDisplayedProductAnalytics] = useState(null)
    const openProductAnalytics = (p) => { setSelectedProductAnalytics(p); setDisplayedProductAnalytics(p) }
...
    useEffect(() => { fetchDashboard() }, [filter, customFrom, customTo])
...
    // Column-header sort for the Stock Levels table: click a header to sort the
...
        setInvSort((prev) => (prev.key === key ? { key, dir: prev.dir === "asc" ? "desc" : "asc" } : { key, dir: "asc" }))
    }
    const displayedInventory = useMemo(() => {
        const list = catalog.filter((c) => c.title.toLowerCase().includes(searchQuery.toLowerCase()))
        const { key, dir } = invSort
        const mul = dir === "asc" ? 1 : -1
        list.sort((a, b) => {
            if (key === "product") return mul * String(a.title || "").localeCompare(String(b.title || ""))
            if (key === "category") return mul * String(a.category || "").localeCompare(String(b.category || ""))
...
            if (key === "status") return mul * statusLabelOf(a.stock).localeCompare(statusLabelOf(b.stock))
...
    // Human-readable label for the active period — shared by the header, exports
...
            : "Custom (select dates)")
        : (FILTERS.find((f) => f.key === filter)?.label || "All Time")
...
        if (rows.length) {
            const range = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: rows.length, c: Object.keys(rows[0]).length - 1 } })
...
            return
        }
        const totalCatRevenue = categoryData.reduce((s, c) => s + c.revenue, 0) || 1

        const summary = [
            ["Products & Orders Analytics"],
            ["Period", periodLabel],
            ...(filter === "custom" ? [["Custom Range", `${customFrom || "—"} to ${customTo || "—"}`]] : []),
...
            ["Highest Revenue", insights.highest?.title || "—", Math.round(insights.highest?.revenue || 0)],
            ["Most Ordered", insights.mostOrdered?.title || "—", insights.mostOrdered?.orders || 0],
            ["Fastest Growing", insights.fastestGrowing?.title || "—", insights.fastestGrowing?.growthHasPrev ? `${insights.fastestGrowing.growth >= 0 ? "+" : ""}${Math.round(insights.fastestGrowing.growth)}%` : "N/A"],
            ["Lowest Performing", insights.lowest?.title || "—", Math.round(insights.lowest?.revenue || 0)],
        ]
...
            "Revenue Share %": revenue ? Math.round((p.revenue / revenue) * 1000) / 10 : 0,
...
            "Revenue Share %": revenue ? Math.round((c.revenue / revenue) * 1000) / 10 : 0,
        }))

        const wb = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(summary), "Summary")
        autoSheet(wb, "Products", products, [6, 32, 16, 10, 8, 12, 14, 10, 10, 8, 8])
        autoSheet(wb, "Category Performance", categories, [16, 12, 14, 8, 10, 12, 10])
        autoSheet(wb, "Revenue Trend", trend, [12, 12, 10])
        autoSheet(wb, "Inventory", inventory, [32, 16, 8, 14, 10, 8, 12, 14])
        XLSX.writeFile(wb, `Products_Analytics_${fileSafe(periodLabel)}_${new Date().toISOString().slice(0, 10)}.xlsx`)
...
            return
        }
        if (filter === "custom" && customFrom && customTo && new Date(customFrom) > new Date(customTo)) {
...
        // Add a fresh page + section title, breaking early if the current page
        // has no room left for a heading + a few rows.
        const sectionTitle = (title, y) => {
            let ty = y
            if (ty > pageH - 100) { doc.addPage(); ty = M }
            doc.setFont("helvetica", "bold"); doc.setFontSize(13); doc.setTextColor(25, 28, 30)
            doc.text(title, M, ty)
            return ty + 16
        }

        doc.setFont("helvetica", "bold"); doc.setFontSize(20); doc.setTextColor(169, 21, 21)
        doc.text("Product & Order Analytics Report", M, M)
        doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(80, 95, 118)
        doc.text(`Period: ${periodLabel}`, M, M + 20)
        doc.text(`Generated: ${new Date().toLocaleString("en-IN")}`, M, M + 36)
        doc.setDrawColor(228, 226, 225)
        doc.line(M, M + 48, rightX, M + 48)
...
            ],
            styles: { font: "helvetica", fontSize: 9, cellPadding: 5, textColor: [40, 40, 50], lineColor: [225, 224, 235], lineWidth: 0.5 },
            headStyles: { fillColor: [169, 21, 21], textColor: [255, 255, 255], fontStyle: "bold" },
...
                body: categoryData.map((c) => [c.name, formatINR(c.revenue), formatNum(c.orders), formatNum(c.products), `${c.growth >= 0 ? "+" : ""}${c.growth}%`]),
                styles: { font: "helvetica", fontSize: 9, cellPadding: 5, textColor: [40, 40, 50], lineColor: [225, 224, 235], lineWidth: 0.5 },
                headStyles: { fillColor: [169, 21, 21], textColor: [255, 255, 255], fontStyle: "bold" },
                alternateRowStyles: { fillColor: [250, 248, 255] },
...
            })
        } else {
            doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(80, 95, 118)
...
        y = sectionTitle(`Products (${displayedProducts.length})`, doc.lastAutoTable.finalY + 28)
        autoTable(doc, {
            startY: y,
            margin: { left: M, right: M },
            head: [["#", "Product", "Category", "Revenue", "Orders", "Units", "Growth %"]],
            body: displayedProducts.length
                ? displayedProducts.map((p, i) => [
                    String(i + 1), p.title, p.category || "—", formatINR(p.revenue),
                    formatNum(p.orders), formatNum(p.units),
                    p.growthHasPrev ? `${p.growth >= 0 ? "+" : ""}${Math.round(p.growth)}%` : "N/A",
                ])
                : [["—", "No products match the current filters.", "", "", "", "", ""]],
            styles: { font: "helvetica", fontSize: 8, cellPadding: 4, textColor: [40, 40, 50], lineColor: [225, 224, 235], lineWidth: 0.5 },
            headStyles: { fillColor: [169, 21, 21], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 8 },
            alternateRowStyles: { fillColor: [250, 248, 255] },
            columnStyles: { 0: { halign: "center", cellWidth: 24 }, 3: { halign: "right" }, 4: { halign: "right" }, 5: { halign: "right" }, 6: { halign: "right" } },
        })

        /* ---- Inventory / Stock Levels (Inventory tab) ---- */
        y = sectionTitle(`Inventory (${displayedInventory.length} SKUs)`, doc.lastAutoTable.finalY + 28)
        autoTable(doc, {
            startY: y,
            margin: { left: M, right: M },
            head: [["Product", "Category", "Stock", "Status", "Units Sold", "Orders", "Revenue"]],
            body: displayedInventory.length
                ? displayedInventory.map((c) => [
                    c.title, c.category || "—", formatNum(c.stock),
                    stockStatus(c.stock) === "out" ? "Out of Stock" : stockStatus(c.stock) === "low" ? "Low Stock" : "In Stock",
                    formatNum(c.unitsSold), formatNum(c.ordersCount), formatINR(c.revenue),
                ])
                : [["No inventory data.", "", "", "", "", "", ""]],
            styles: { font: "helvetica", fontSize: 8, cellPadding: 4, textColor: [40, 40, 50], lineColor: [225, 224, 235], lineWidth: 0.5 },
            headStyles: { fillColor: [169, 21, 21], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 8 },
...
            doc.text(`Page ${i} of ${pageCount}`, rightX, pageH - 20, { align: "right" })
        }

        doc.save(`Product_Analytics_Report_${fileSafe(periodLabel)}_${new Date().toISOString().slice(0, 10)}.pdf`)
...
        <div className="kpi-card bg-surface rounded-xl p-4 flex flex-col gap-2" style={{ border: `1px solid ${C.border}`, boxShadow: "0px 1px 2px rgba(0,0,0,0.05)" }}>
...
        <button className="flex items-center gap-3 w-full rounded-lg text-sm font-medium" style={{ padding: "8px 16px", background: active ? C.activeNav : "transparent", color: active ? "var(--color-inverse)" : C.textS }}>
...
                                    <div className="flex flex-col gap-2">{bar({ width: "220px", height: "24px" })}{bar({ width: "300px", height: "14px" })}</div>
                                    <div className="flex items-center gap-3">{bar({ width: "120px", height: "40px", borderRadius: "8px" })}{bar({ width: "110px", height: "40px", borderRadius: "8px" })}</div>
                                </div>
                                {/* KPI cards */}
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
                                    {[...Array(6)].map((_, i) => (
                                        <div key={i} className="bg-surface rounded-xl p-4 flex flex-col gap-3" style={{ border: `1px solid ${C.border}` }}>
                                            <div className="flex items-center justify-between">{bar({ width: "32px", height: "32px", borderRadius: "8px" })}{bar({ width: "44px", height: "14px" })}</div>
...
                                    <div className="lg:col-span-2 bg-surface rounded-xl p-6" style={{ border: `1px solid ${C.border}` }}>
                                        <div className="flex items-center justify-between mb-4">{bar({ width: "150px", height: "18px" })}{bar({ width: "160px", height: "12px" })}</div>
                                        {bar({ width: "100%", height: "320px", borderRadius: "12px" })}
                                    </div>
                                    <div className="bg-surface rounded-xl p-6" style={{ border: `1px solid ${C.border}` }}>
                                        {bar({ width: "140px", height: "18px", marginBottom: "20px" })}
                                        <div className="flex flex-col gap-4">
                                            {[...Array(5)].map((_, i) => <div key={i} className="flex items-center gap-3">{bar({ width: "36px", height: "36px", borderRadius: "8px" })}<div className="flex flex-col gap-2 flex-1">{bar({ width: "70%", height: "12px" })}{bar({ width: "40%", height: "10px" })}</div></div>)}
                                        </div>
                                    </div>
                                </div>
                                {/* Inventory table */}
                                <div className="bg-surface rounded-xl overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
                                    <div className="flex items-center gap-4 px-4 py-3" style={{ background: "var(--color-surface-muted)" }}>
...
                                            {[...Array(7)].map((_, i) => <div key={i} style={{ flex: 1 }}>{bar({ width: i === 0 ? "80%" : "60%", height: "14px" })}</div>)}
                                        </div>
                                    ))}
                                </div>
                            </div>
...
                                </button>
                                <button onClick={exportToExcel} title="Export to Excel" className="flex items-center gap-2 rounded-lg px-4 text-sm font-semibold text-inverse" style={{ background: C.indigo, height: "40px" }}>
...
                        {/* STICKY FILTER BAR */}
                        <div className="sticky top-0 z-10 py-4 mb-6 flex flex-col gap-3" style={{ background: "color-mix(in srgb, var(--color-background) 85%, transparent)", backdropFilter: "blur(6px)", borderBottom: `1px solid color-mix(in srgb, var(--color-border) 40%, transparent)` }}>
                            <div className="flex flex-wrap items-center gap-1">
                                {FILTERS.map((f) => (
                                    <button key={f.key} onClick={() => setFilter(f.key)} className="text-xs font-medium rounded-full"
                                        style={filter === f.key ? { background: C.indigo, color: "var(--color-inverse)", padding: "5px 16px" } : { background: "var(--color-surface)", color: C.textS, border: `1px solid ${C.border}`, padding: "4px 16px" }}>
                                        {f.label}
                                    </button>
                                ))}
                                {filter === "custom" && (
                                    <div className="flex items-center gap-2 ml-1">
                                        <input type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} max={customTo || undefined} className="outline-none rounded-lg text-xs px-2 py-1.5 bg-surface" style={{ border: `1px solid ${C.border}` }} />
                                        <span className="text-xs" style={{ color: C.textS }}>to</span>
                                        <input type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} min={customFrom || undefined} className="outline-none rounded-lg text-xs px-2 py-1.5 bg-surface" style={{ border: `1px solid ${C.border}` }} />
...
                                    <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search products..." className="w-full outline-none rounded-lg text-sm bg-surface" style={{ border: `1px solid ${C.border}`, padding: "9px 16px 10px 32px" }} />
                                </div>
                                <div className="flex items-center gap-2 rounded-lg px-3 bg-surface" style={{ border: `1px solid ${C.border}` }}>
...
                                    </select>
                                </div>
                                <button className="flex items-center gap-2 rounded-lg px-3 text-sm bg-surface" style={{ border: `1px solid ${C.border}`, color: C.textS }}>
...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                                    {/* Revenue Trend */}
                                    <div className="chart-container lg:col-span-2 bg-surface rounded-xl" style={{ border: `1px solid ${C.border}` }}>
...
                                                    <CartesianGrid vertical={false} stroke="var(--color-chart-grid)" strokeOpacity={0.2} />
                                                    <XAxis dataKey="date" tick={{ fontSize: 11, fill: "var(--color-chart-axis)" }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                                                    <YAxis tick={{ fontSize: 11, fill: "var(--color-chart-axis)" }} tickLine={false} axisLine={false} tickFormatter={(v) => (v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v)} width={44} />
                                                    <Tooltip formatter={(v) => formatINR(v)} />
                                                    <Bar dataKey="revenue" fill="color-mix(in srgb, var(--color-chart-1) 20%, transparent)" radius={[2, 2, 0, 0]} />
                                                </BarChart>
                                            </ResponsiveContainer>
                                        </div>
...
                                    <div className="chart-container bg-surface rounded-xl p-6" style={{ border: `1px solid ${C.border}` }}>
...
                                                    </div>
                                                    <div className="rounded-full overflow-hidden" style={{ height: "8px", background: "var(--color-surface-muted)" }}>
...
                                        badge={insights.fastestGrowing ? { text: `+${Math.abs(Math.round(insights.fastestGrowing.growth))}%`, bg: "color-mix(in srgb, var(--color-accent) 10%, transparent)", color: C.indigo } : null} />
                                    <InsightCard label="LOWEST PERFORMING" labelColor={C.red} product={insights.lowest}
                                        badge={insights.lowest ? { text: formatINR(insights.lowest.revenue), bg: "color-mix(in srgb, var(--color-error) 10%, transparent)", color: C.red } : null} />
                                </div>
...
                                        <button onClick={() => setActiveTab("analytics")} className="text-sm font-medium" style={{ color: C.indigo }}>View All Products</button>
                                    </div>
...
                                                    const rank = i + 1
                                                    const badge = rank === 1 ? { bg: "var(--color-warning-subtle)", color: "var(--color-chart-gold)" } : rank === 2 ? { bg: "var(--color-surface-muted)", color: "var(--color-chart-silver)" } : rank === 3 ? { bg: "var(--color-accent-subtle)", color: "var(--color-chart-bronze)" } : null
                                                    return (
...
                                                            <td className="px-4 py-3">
                                                                <div className="flex items-center gap-3">
                                                                    <ProductThumb src={p.image} alt={p.title} size={40} />
...
                                                                {p.rating ? <span className="inline-flex items-center gap-1"><Star size={13} style={{ color: "var(--color-accent)" }} fill="var(--color-accent)" />{p.rating.toFixed(1)}</span> : <span style={{ color: C.textS }}>—</span>}
                                                            </td>
...
                                                        <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: pos ? "color-mix(in srgb, var(--color-accent-subtle) 50%, transparent)" : "color-mix(in srgb, var(--color-error-subtle) 40%, transparent)", color: pos ? C.indigo : C.red }}>{pos ? "+" : ""}{cat.growth}%</span>
                                                    </div>
...
                                    </div>
                                    <div className="flex rounded-lg overflow-hidden" style={{ background: "var(--color-surface-muted)", border: `1px solid ${C.border}` }}>
                                        {["grid", "list"].map((m) => (
                                            <button key={m} onClick={() => setViewMode(m)} className="px-4 py-1.5 text-sm font-medium capitalize" style={viewMode === m ? { background: C.indigo, color: "var(--color-inverse)" } : { color: C.textS }}>{m}</button>
                                        ))}
                                    </div>
                                </div>
...
                                                        <span className="absolute top-2 left-2 text-xs font-semibold text-inverse rounded px-2 py-0.5 capitalize" style={{ background: cs.bg === "var(--color-info-subtle)" ? C.textS : cs.bg }}>{p.category}</span>
                                                        {p.rating > 0 && (
                                                            <span className="absolute top-2 right-2 flex items-center gap-1 text-xs font-semibold rounded px-2 py-0.5" style={{ background: "color-mix(in srgb, var(--color-background) 90%, transparent)", backdropFilter: "blur(2px)", color: C.textP }}>
                                                                <Star size={11} style={{ color: "var(--color-accent)" }} fill="var(--color-accent)" />{p.rating.toFixed(1)}
                                                            </span>
                                                        )}
                                                    </div>
...
                                                        </div>
                                                        <button onClick={() => openProductAnalytics(p)} className="w-full flex items-center justify-center gap-1 rounded-lg py-2 text-sm font-bold" style={{ background: "var(--color-info-subtle)", color: C.textP }}>View Analytics <ArrowRight size={14} /></button>
                                                    </div>
                                                </div>
                                            )
                                        })}
...
                                                <ProductThumb src={p.image} alt={p.title} size={40} radius={8} />
...
                                                        const justify = col.align === "right" ? "justify-end" : col.align === "center" ? "justify-center" : ""
                                                        return (
                                                            <th
                                                                key={col.key}
...
                                                                    {col.label}
                                                                    {active ? (invSort.dir === "asc" ? <ArrowUp size={12} /> : <ArrowDown size={12} />) : <ArrowUpDown size={11} style={{ opacity: 0.35 }} />}
                                                                </span>
                                                            </th>
                                                        )
                                                    })}
...
                                                    const st = stockStatus(c.stock)
                                                    const badge = st === "in" ? { t: "In Stock", bg: "color-mix(in srgb, var(--color-accent) 10%, transparent)", col: C.indigo } : st === "low" ? { t: "Low Stock", bg: "var(--color-warning-subtle)", col: "var(--color-warning)" } : { t: "Out of Stock", bg: "color-mix(in srgb, var(--color-error) 10%, transparent)", col: C.red }
                                                    return (
                                                        <tr key={c.id} className="product-row" style={{ borderTop: `1px solid ${C.border}` }}>
                                                            <td className="px-4 py-3">
                                                                <div className="flex items-center gap-3">
                                                                    <ProductThumb src={c.image} alt={c.title} size={40} />
...
                                        <p className="text-sm" style={{ color: C.textS }}>Summary for {periodLabel}</p>
                                    </div>
                                    <button onClick={downloadReportPDF} className="flex items-center gap-2 rounded-lg px-4 text-sm font-semibold text-inverse" style={{ background: C.indigo, height: "40px" }}>
                                        <FileDown size={16} /> Download Report
                                    </button>
                                </div>

                                {/* Summary metrics */}
                                <div className="bg-surface rounded-xl p-6 mb-6" style={{ border: `1px solid ${C.border}` }}>
...
                                                    </div>
                                                    <div className="rounded-full overflow-hidden" style={{ height: "6px", background: "var(--color-surface-muted)" }}>
...
                            style={activeTab === t.k ? { background: C.indigo, color: "var(--color-inverse)", padding: "6px 16px" } : { background: "var(--color-surface)", color: C.textS, border: `1px solid ${C.border}`, padding: "5px 16px" }}>
                            {t.l}
                        </button>
                    ))}
                </div>
...
                            style={filter === f.key ? { background: C.activeNav, color: "var(--color-inverse)", padding: "4px 12px" } : { background: "var(--color-surface)", color: C.textS, border: `1px solid ${C.border}`, padding: "3px 12px" }}>
                            {f.label}
                        </button>
                    ))}
                </div>
...
                                <div className="flex flex-col gap-2 mb-4">{bar({ width: "180px", height: "26px" })}{bar({ width: "240px", height: "13px" })}</div>
                                {/* KPI grid */}
                                <div className="grid grid-cols-2 gap-3 mb-5">
                                    {[...Array(4)].map((_, i) => (
                                        <div key={i} className="rounded-2xl bg-surface p-4 flex flex-col gap-2" style={{ border: `1px solid ${C.border}` }}>
                                            {bar({ width: "32px", height: "32px", borderRadius: "8px" })}
...
                                {/* Chart cards */}
                                {[...Array(2)].map((_, i) => (
                                    <div key={i} className="rounded-2xl bg-surface p-4 mb-4" style={{ border: `1px solid ${C.border}` }}>
                                        <div className="flex items-center justify-between mb-3">{bar({ width: "150px", height: "18px" })}{bar({ width: "40px", height: "18px" })}</div>
                                        {bar({ width: "100%", height: "180px", borderRadius: "12px" })}
                                    </div>
                                ))}
                            </div>
                        )
...
                                {/* Daily Revenue Trend */}
                                <div className="rounded-2xl bg-surface p-4 mb-4" style={{ border: `1px solid ${C.border}` }}>
...
                                                <Tooltip formatter={(v) => formatINR(v)} cursor={{ fill: "color-mix(in srgb, var(--color-chart-1) 8%, transparent)" }} />
...
                                        <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "var(--color-surface-muted)", color: C.indigo }}>Top 5</span>
                                    </div>
                                    <div className="flex flex-col gap-3">
                                        {ordersBarData.map((p) => (
                                            <div key={p.id}>
...
                                                <div className="rounded-full overflow-hidden" style={{ height: "8px", background: "var(--color-surface-muted)" }}><div className="h-full rounded-full" style={{ width: `${(p.orders / maxOrders) * 100}%`, background: C.activeNav }} /></div>
                                            </div>
                                        ))}
                                        {ordersBarData.length === 0 && <p className="text-sm" style={{ color: C.textS }}>No order data.</p>}
                                    </div>
...
                                        <div key={p.id} className="flex-shrink-0 rounded-2xl bg-surface overflow-hidden" style={{ width: "160px", border: `1px solid ${C.border}` }}>
                                            <div style={{ height: "110px" }}><ProductThumb src={p.image} alt={p.title} fill /></div>
                                            <div className="p-3">
...
                                        return (
                                            <div key={cat.name} className="flex-shrink-0 rounded-2xl p-4" style={{ width: "200px", background: "var(--color-surface)", border: `1px solid ${C.border}` }}>
                                                <div className="flex items-center justify-between mb-3">
                                                    <span className="flex items-center justify-center rounded-lg p-2" style={{ background: cs.bg }}><Icon size={18} style={{ color: cs.icon }} /></span>
...
                                    <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "var(--color-muted)" }} />
                                    <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search products..." className="w-full outline-none rounded-xl text-sm bg-surface" style={{ border: `1px solid ${C.border}`, padding: "10px 12px 10px 34px" }} />
                                </div>
                                <div className="flex flex-col gap-3">
                                    {displayedProducts.slice(0, 20).map((p) => (
                                        <div key={p.id} onClick={() => openProductAnalytics(p)} className="flex items-center gap-3 rounded-2xl bg-surface p-3 cursor-pointer" style={{ border: `1px solid ${C.border}` }}>
                                            <ProductThumb src={p.image} alt={p.title} size={64} radius={8} />
...
                                        { l: "Low", v: inventoryStats.lowStock, c: "var(--color-warning)" },
                                        { l: "Out", v: inventoryStats.outStock, c: C.red },
                                    ].map((s) => (
                                        <div key={s.l} className="rounded-xl bg-surface p-3 text-center" style={{ border: `1px solid ${C.border}` }}>
...
                                        return (
                                            <div key={c.id} className="flex items-center gap-3 rounded-2xl bg-surface p-3" style={{ border: `1px solid ${C.border}` }}>
                                                <ProductThumb src={c.image} alt={c.title} size={72} radius={8} />
                                                <div className="flex-1 min-w-0">
...
                                        <p className="text-sm" style={{ color: C.textS }}>{periodLabel}</p>
                                    </div>
                                    <button onClick={downloadReportPDF} className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-inverse" style={{ background: C.indigo }}><FileDown size={15} /> Export</button>
                                </div>
...
                                        { l: "Active Products", v: formatNum(activeProducts) },
                                    ].map((s) => (
                                        <div key={s.l} className="rounded-xl bg-surface p-3" style={{ border: `1px solid ${C.border}` }}>
                                            <p className="text-xs" style={{ color: C.textS }}>{s.l}</p>
                                            <p className="font-bold" style={{ fontSize: "18px", color: C.textP }}>{s.v}</p>
                                        </div>
                                    ))}
...
                                <div className="rounded-2xl bg-surface p-4" style={{ border: `1px solid ${C.border}` }}>
...
                                                <div className="rounded-full overflow-hidden" style={{ height: "6px", background: "var(--color-surface-muted)" }}><div className="h-full rounded-full" style={{ width: `${categoryData[0]?.revenue ? (c.revenue / categoryData[0].revenue) * 100 : 0}%`, background: C.activeNav }} /></div>
                                            </div>
                                        ))}
                                        {categoryData.length === 0 && <p className="text-sm" style={{ color: C.textS }}>No category data.</p>}
                                    </div>
...
            <RightDrawer open={!!selectedProductAnalytics} onClose={closeProductAnalytics} title="Product Analytics">
                {displayedProductAnalytics && (
                    <>
                        <div className="rounded-xl overflow-hidden" style={{ height: "180px" }}>
                            <ProductThumb src={displayedProductAnalytics.image} alt={displayedProductAnalytics.title} fill />
                        </div>
                        <div>
                            <p className="text-xs font-semibold uppercase capitalize" style={{ color: C.indigo, letterSpacing: "0.6px" }}>{displayedProductAnalytics.category || "Uncategorized"}</p>
                            <h3 className="font-bold mt-1" style={{ fontSize: "20px", color: C.textP }}>{displayedProductAnalytics.title}</h3>
                        </div>
                        <div className="flex items-center justify-between rounded-xl p-4" style={{ background: C.sidebarBg }}>
                            <div>
                                <p className="text-xs" style={{ color: C.textS }}>Price</p>
                                <p className="font-bold text-lg" style={{ color: C.textP }}>{formatINR(displayedProductAnalytics.price || displayedProductAnalytics.revenue)}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-xs" style={{ color: C.textS }}>Growth</p>
                                <p className="font-bold text-lg" style={{ color: displayedProductAnalytics.growth >= 0 ? C.indigo : C.red }}>
                                    {displayedProductAnalytics.growthHasPrev ? `${displayedProductAnalytics.growth >= 0 ? "+" : ""}${Math.round(displayedProductAnalytics.growth)}%` : "N/A"}
                                </p>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="rounded-xl p-3" style={{ border: `1px solid ${C.border}` }}>
                                <p className="text-xs" style={{ color: C.textS }}>Revenue</p>
                                <p className="font-bold" style={{ fontSize: "18px", color: C.textP }}>{formatINR(displayedProductAnalytics.revenue)}</p>
                            </div>
                            <div className="rounded-xl p-3" style={{ border: `1px solid ${C.border}` }}>
                                <p className="text-xs" style={{ color: C.textS }}>Orders</p>
                                <p className="font-bold" style={{ fontSize: "18px", color: C.textP }}>{formatNum(displayedProductAnalytics.orders)}</p>
                            </div>
                            <div className="rounded-xl p-3" style={{ border: `1px solid ${C.border}` }}>
                                <p className="text-xs" style={{ color: C.textS }}>Units Sold</p>
                                <p className="font-bold" style={{ fontSize: "18px", color: C.textP }}>{formatNum(displayedProductAnalytics.units)}</p>
                            </div>
                            <div className="rounded-xl p-3" style={{ border: `1px solid ${C.border}` }}>
                                <p className="text-xs" style={{ color: C.textS }}>Stock</p>
                                <p className="font-bold" style={{ fontSize: "18px", color: (displayedProductAnalytics.stock ?? 1) <= 0 ? C.red : C.textP }}>{displayedProductAnalytics.stock ?? "—"}</p>
                            </div>
                        </div>
                        {displayedProductAnalytics.rating > 0 && (
                            <div className="flex items-center gap-2">
                                <Star size={16} style={{ color: "var(--color-accent)" }} fill="var(--color-accent)" />
                                <span className="font-semibold" style={{ color: C.textP }}>{displayedProductAnalytics.rating.toFixed(1)}</span>
                                <span className="text-sm" style={{ color: C.textS }}>rating</span>
                            </div>
                        )}
                    </>
                )}
...
    <div className="kpi-card rounded-2xl bg-surface p-4" style={{ border: `1px solid ${C.border}` }}>
        <div className="flex items-center justify-between mb-2">
            <span className="flex items-center justify-center rounded-lg p-1.5" style={{ background: C.lightIndigo }}>{icon}</span>
            {t && <span className="text-xs font-semibold rounded px-1.5 py-0.5" style={{ background: t.up ? "color-mix(in srgb, var(--color-accent) 10%, transparent)" : "color-mix(in srgb, var(--color-error) 10%, transparent)", color: t.up ? C.indigo : C.red }}>{t.up ? "+" : "-"}{t.pct}%</span>}
        </div>
...
        ? { background: "color-mix(in srgb, var(--color-surface) 70%, transparent)", border: "1px solid color-mix(in srgb, var(--color-accent) 20%, transparent)", backdropFilter: "blur(5px)" }
