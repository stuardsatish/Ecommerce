...
function clientParse(text) {
  ...
  const pick = (line) => line.slice(line.indexOf(":") + 1).trim(); // Keep only digits and decimal point — fixes ₹8.18 being parsed as 818
    ...
    if (m) {
      ...
      while (j < lines.length) {
        ...
        if (nl.startsWith("quantity:")) {
          qty = parseInt(lines[j].slice(lines[j].indexOf(":") + 1), 10) || 0;
          j++;
          ...
          /^\d+[.)\s]/.test(lines[j]) ||
          /^(total|subtotal|shipping|promo)/.test(nl)
        )
          break;
        else j++;
      }
...
const CreateOrdersPage = () => {
  ...
  const handleSubmit = useCallback(
    async (allowPriceOverride = false) => {
      ...
      try {
        ...
        const data = await res.json().catch(() => ({})); // Server refused because the pasted total ≠ the live catalog total.
        ...
        if (res.status === 409 && data.code === "PRICE_MISMATCH") {
          ...
          setResult({
            success: false,
            error: "Order cancelled — price mismatch not confirmed.",
          });
          return;
        }
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: "36px" }}>
          ...
          >
                        Paste a WhatsApp order message. The backend will parse,
            validate, and create the order             using the same pipeline
            as online payments.          {" "}
          </p>
                 {" "}
        </div>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  height: "52px",
                  background:
                    loading || !hasContent ? "var(--color-disabled)" : C.brand,
                  color: C.white,
                  border: "none",
                  borderRadius: "12px",
                  fontFamily: MANROPE,
              ...
              <button
                ...
                onMouseLeave={(e) => {
                  ...
                  e.currentTarget.style.color = C.textS;
                }}
              >
                                <Trash2 size={18} />             {" "}
              </button>
                ...
                {result.success && (
                  ...
                  >
                    ...
                    {result.parsedOrder?.isGuest && (
                      ...
                      >
                                                Created as guest order — no
                        matching Firebase user found.                      {" "}
                      </span>
                    )}
                                     {" "}
                  </div>
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    {paymentStatus === "paid" ? (
                      ...
                      <>
                        <Clock size={14} color="var(--color-accent-strong)" />
                        <span
                          style={{
                            fontSize: "12px",
                            color: "var(--color-accent-strong)",
            ...
            >
              ...
              >
                ...
                <br />                The server parses, matches each product to
                the catalog,                 validates stock, deducts inventory,
                creates the order,                 updates analytics, clears the
                customer's cart, and records                 the revenue —
                identical to an online payment.              {" "}
              </p>
                         {" "}
            </div>
                     {" "}
