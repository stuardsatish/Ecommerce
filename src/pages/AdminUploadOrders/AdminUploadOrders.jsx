...
const AdminUploadOrders = () => {
  ...
  const intervalRef =
    useRef(null); /* ---------------- REALTIME LISTENERS ---------------- */

  useEffect(() => {
    const unsubSessions = onSnapshot(
      query(
        collection(fireDB, "uploadSessions"),
        orderBy("createdAt", "desc"),
        limit(10),
      ),
      (snap) => {
        setUploadSessions(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
  ...
  const kpis = useMemo(() => {
    ...
    const todayImports = uploadSessions.filter((s) => {
    ...
    const totalRecords = uploadSessions.reduce(
      (a, s) => a + (Number(s.recordCount) || 0),
      0,
    );
    const totalSuccess = uploadSessions.reduce(
      (a, s) => a + (Number(s.successCount) || 0),
      0,
    );
    const successRate =
      totalRecords > 0
  ...
  const chartData = useMemo(() => {
    if (importAnalytics.length) {
      const max = Math.max(
        ...importAnalytics.map((d) => Number(d.count) || 0),
        1,
      );
      return {
        bars: importAnalytics.map((d) => ({
          label: d.date,
  ...
  const logActivity = (event, type) =>
    addDoc(collection(fireDB, "activityLog"), {
      event,
      actor: actorName,
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const rows = results.data.map((r) => {
          ...
          const isValidPrice =
            !isNaN(parseFloat(r.price)) && parseFloat(r.price) > 0;
          const isValidQty = !isNaN(parseInt(r.qty)) && parseInt(r.qty) > 0;
          const valid =
            isValidId && isValidCustomer && isValidPrice && isValidQty;
          return {
            order_id: orderId,
        ...
        setIdsSanitized(
          withId.length > 0 && withId.every((r) => r.status === "VALID"),
        );
        setPreviewData(rows);
  ...
  const finishUpload = async () => {
    try {
      ...
      try {
        ...
        usersSnap.forEach((d) => {
          const u = d.data();
          userMap[d.id] = { name: u.name || "User", email: u.email || "" };
        });
      } catch {
      ...
      Object.values(groups).forEach((rws) => {
        ...
        const day = now.toISOString().slice(0, 10),
          month = day.slice(0, 7),
          year = day.slice(0, 4);
        (dailyAgg[day] = dailyAgg[day] || { revenue: 0, orders: 0 }).revenue +=
          total;
        dailyAgg[day].orders++;
        (monthlyAgg[month] = monthlyAgg[month] || {
          revenue: 0,
          orders: 0,
        }).revenue += total;
        monthlyAgg[month].orders++;
        (yearlyAgg[year] = yearlyAgg[year] || {
          revenue: 0,
          orders: 0,
        }).revenue += total;
        yearlyAgg[year].orders++;
        const ca = (custAgg[uid] = custAgg[uid] || {
          name: userMap[uid]?.name || "Imported User",
        ...
        ca.spent += total;
        products.forEach((p) => {
          const pa = (prodAgg[p.productId] = prodAgg[p.productId] || {
            title: p.title,
            category: p.category,
            orders: 0,
            revenue: 0,
      ...
      for (const [day, v] of Object.entries(dailyAgg))
        await setDoc(
      ...
      for (const [month, v] of Object.entries(monthlyAgg))
        await setDoc(
          doc(fireDB, "analytics", "monthly", "stats", month),
          { month, revenue: increment(v.revenue), orders: increment(v.orders) },
          { merge: true },
        );
      for (const [year, v] of Object.entries(yearlyAgg))
        await setDoc(
          doc(fireDB, "analytics", "yearly", "stats", year),
          { year, revenue: increment(v.revenue), orders: increment(v.orders) },
          { merge: true },
        );
      for (const [pid, v] of Object.entries(prodAgg))
        await setDoc(
          doc(fireDB, "productStats", pid),
          {
            title: v.title,
            category: v.category,
            totalOrders: increment(v.orders),
            totalRevenue: increment(v.revenue),
            totalQuantity: increment(v.qty),
            lastSoldAt: Timestamp.fromDate(now),
          },
          { merge: true },
        );
      for (const [uid, v] of Object.entries(custAgg)) {
        ...
        const newO =
          (cSnap.exists() ? cSnap.data().totalOrders || 0 : 0) + v.orders;
        const newS =
          (cSnap.exists() ? cSnap.data().totalSpent || 0 : 0) + v.spent;
        await setDoc(
          cRef,
          {
            name: v.name,
            ...
            totalOrders: newO,
            totalSpent: newS,
            avgOrderValue: newO > 0 ? Math.round(newS / newO) : 0,
            lastOrderDate: Timestamp.fromDate(now),
          },
          { merge: true },
        );
      }
      for (const lg of invLogs)
        await addDoc(collection(fireDB, "inventoryLogs"), lg); // Upload session (drives Upload History + KPIs)
      ...
      await logActivity(
        `Imported ${orderDocs.length} order(s) from ${selectedFile.name}`,
        "upload",
      );
  ...
  const Toggle = ({ on, onClick }) => (
    <button
      ...
      style={{
        ...
        height: "20px",
        borderRadius: "9999px",
        background: on ? "var(--color-primary)" : "var(--color-surface-muted)",
        position: "relative",
  ...
  const statusVisual = (s) => {
    ...
    if (s === "active" || s === "pending")
      return {
        ...
        Icon: Upload,
        label: s === "pending" ? "PENDING" : "ACTIVE",
      };
    return {
  ...
  const dotColor = (t) =>
    ({
      ...
      warning: "var(--color-accent-strong)",
  ...
  const configCards = [
    {
      ...
      sub: "Overwrite duplicate IDs",
    },
    {
      key: "appendRecords",
      title: "Append Records",
      sub: "Add as new entries",
    },
    { key: "autoValidate", title: "Auto Validate", sub: "AI-powered checks" },
  ];

  const visiblePreview = previewData.slice(
    0,
    10,
  ); /* ============================== RENDER ============================== */
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen p-8 pt-[96px] sm:pt-8"
      style={{ background: "var(--color-background)", fontFamily: INTER }}
    >
            {/* SECTION 1 — KPI CARDS */}     {" "}
      <div
        ...
        {[
          {
            ...
            value: kpis.todayImports.toLocaleString(),
            trend: (
              <>
                <ArrowUp size={10} style={{ color: "var(--color-primary)" }} />
                <span
                  style={{
      ...
      <div className="flex flex-col lg:flex-row" style={{ gap: "24px" }}>
                {/* LEFT COLUMN */}       {" "}
        <div
          className="flex flex-col min-w-0 lg:flex-1"
          style={{ gap: "24px", width: "100%" }}
        >
                    {/* UPLOAD CARD */}         {" "}
          <div
            className="upload-card"
            style={{ ...glass, borderRadius: "48px", padding: "32px" }}
          >
                       {" "}
            <div
            ...
            >
              ...
              <div className="flex items-center" style={{ gap: "16px" }}>
                ...
                               {" "}
                <span style={{ fontSize: "16px", color: "var(--color-ink)" }}>
                  Bulk Order Import
                </span>
                             {" "}
              </div>
            ...
            <div
              ...
              style={{
                height: "320px",
                borderRadius: "32px",
                border: `3px dashed ${isDragOver ? "var(--color-primary)" : "var(--color-border)"}`,
            ...
            >
              ...
              {selectedFile ? (
                <>
                  ...
                  >
                    {bytesToMB(selectedFile.size)} MB • {previewData.length}{" "}
                    rows parsed
                  </span>
                                 {" "}
                </>
                ...
                <>
                  ...
                  <span
                    style={{ fontSize: "14px", color: "var(--color-body)" }}
                  >
                    Or click to browse from your computer. Maximum file size is
                    50 MB.
                  </span>
                                   {" "}
                  <button
                ...
                <tbody>
                  ...
                  {previewData.length === 0 && (
                    <tr>
                      ...
                      >
                        Select a CSV file to preview and validate rows.
                      </td>
                    </tr>
                  )}
                                 {" "}
        ...
               {" "}
        <div
          className="flex flex-col gap-6 lg:flex-shrink-0 lg:max-w-[384px]"
          style={{ width: "100%" }}
        >
                    {/* UPLOAD HISTORY */}         {" "}
          <div
            className="sidebar-card"
            style={{ ...glass, borderRadius: "32px", padding: "24px" }}
          >
                       {" "}
            <div
              className="flex items-center justify-between"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-1 min-w-0">
                      ...
                      >
                        {bytesToMB(s.fileSize)} MB •{" "}
                        {relativeTime(toJsDate(s.createdAt))}
                      </span>
                                         {" "}
                    </div>
                                       {" "}
          ...
          >
            ...
            <div
              ...
              style={{ height: "160px", padding: "20px 0 12px", gap: "8px" }}
            >
                           {" "}
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: isCurrent
                          ? "var(--color-primary)"
                          : `color-mix(in srgb, var(--color-accent) ${Math.round((0.1 + (b.count / chartData.max) * 0.3) * 100)}%, transparent)`,
                        boxShadow: isCurrent
                          ? "0px 10px 15px -3px color-mix(in srgb, var(--color-accent) 30%, transparent)"
                          : "none",
                      }}
                    />
                                       {" "}
          ...
                    {/* CSV TEMPLATE */}         {" "}
          <div
            className="sidebar-card relative"
            style={{
              background: "var(--color-primary)",
              border:
                "1px solid color-mix(in srgb, var(--color-surface) 50%, transparent)",
              boxShadow: "0px 10px 30px rgba(0,0,0,0.03)",
              borderRadius: "32px",
              padding: "24px",
              minHeight: "307px",
            ...
            <div style={{ position: "relative", zIndex: 2 }}>
              ...
              <p
                style={{
                  ...
                  fontSize: "12px",
                  color:
                    "color-mix(in srgb, var(--color-inverse) 80%, transparent)",
                  lineHeight: "16px",
                  marginBottom: "16px",
                }}
              >
                Ensure your data matches our system schema by downloading our
                master template.
              </p>
                           {" "}
              <div
              ...
              >
                ...
                >
                                   {" "}
                  {["order_id", "customer_email", "sku", "unit_price"].map(
                    (c) => (
                      <span
                        key={c}
                        style={{
          ...
          >
            ...
            <div
              ...
              style={{ gap: "24px", isolation: "isolate" }}
            >
                           {" "}
