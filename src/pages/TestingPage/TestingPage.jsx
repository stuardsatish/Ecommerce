...
  { fileName: "Q3_Final_Orders.csv", status: "success", recordCount: 1245, successCount: 1241, failCount: 4 },
  { fileName: "wholesale_batch_A.csv", status: "active", recordCount: 320, successCount: 0, failCount: 0 },
  { fileName: "inventory_sync_fail.csv", status: "failed", recordCount: 88, successCount: 0, failCount: 88 },
  { fileName: "retail_partners.csv", status: "success", recordCount: 540, successCount: 538, failCount: 2 },
  { fileName: "promo_signups.csv", status: "pending", recordCount: 150, successCount: 0, failCount: 0 },
]
...
  const busy = loadingData || seeding || deleting || backfilling || seedingUploads || removingUploads
...
        const u = d.data()
        return { uid: d.id, name: u.name || "User", email: u.email || `${d.id}@test.com` }
      })
...
        )
        log(`ERROR: insufficient data (${userList.length} users, ${productList.length} products)`)
        return
      }

      setDataLoaded(true)
      log(`Loaded ${userList.length} users and ${productList.length} products ✓`)
    } catch (err) {
      console.error(err)
...
      await batch.commit()
      if (onProgress) onProgress(Math.min(100, Math.round(((i + BATCH_CHUNK) / ops.length) * 100)))
    }
  }
...
    setProgress(0)
    try {
      log(`Building ${ORDER_COUNT} orders in memory…`)

      // 1. Build all orders in memory
      const now = new Date()
      const orders = []
...
          category: p.category,
        }))
        const total = Math.round(items.reduce((s, it) => s + it.price * it.quantity, 0))
        const totalItems = items.reduce((s, it) => s + it.quantity, 0)
        const u = pickOne(users)
        const status = lifecycleStatus(date, now)
        // Delivered orders get a delivery date 4–5 days after they were placed.
        const deliveredAt = status === "delivered" ? new Date(date.getTime() + randInt(4, 5) * DAY_MS) : null
        orders.push({ user: u, items, total, totalItems, date, status, deliveredAt })
      }

      // 2. Aggregate analytics in memory
      const daily = {}, monthly = {}, yearly = {}
      const prodStats = {}, stockDec = {}, custStats = {}
...
        const cu = o.user
        if (!custStats[cu.uid]) custStats[cu.uid] = { name: cu.name, email: cu.email, totalOrders: 0, totalSpent: 0, lastOrderDate: o.date }
        custStats[cu.uid].totalOrders += 1
        custStats[cu.uid].totalSpent += o.total
        if (o.date > custStats[cu.uid].lastOrderDate) custStats[cu.uid].lastOrderDate = o.date
        cartUsers.add(cu.uid)

        o.items.forEach((it) => {
          if (!prodStats[it.productId]) prodStats[it.productId] = { title: it.title, category: it.category || "general", totalOrders: 0, totalRevenue: 0, totalQuantity: 0, lastSoldAt: o.date }
...
          if (o.date > prodStats[it.productId].lastSoldAt) prodStats[it.productId].lastSoldAt = o.date
...
      // A. orders (top-level, like CartPage) + per-user subcollection (where the
...
          ...(o.deliveredAt ? { deliveredAt: Timestamp.fromDate(o.deliveredAt) } : {}),
          products: o.items,
          _seeded: true,
        }
        ops.push((b) => b.set(orderRef, orderData))
        // Mirror into /users/{uid}/orders/{orderId} — this is what the
...
        ops.push((b) =>
          b.set(doc(fireDB, "analytics", "daily", "stats", day), {
            date: day,
...
        ops.push((b) =>
          b.set(doc(fireDB, "analytics", "monthly", "stats", month), {
            month,
            revenue: increment(v.revenue),
            orders: increment(v.orders),
          }, { merge: true })
...
        ops.push((b) =>
          b.set(doc(fireDB, "analytics", "yearly", "stats", year), {
            year,
            revenue: increment(v.revenue),
            orders: increment(v.orders),
          }, { merge: true })
...
        ops.push((b) =>
          b.set(doc(fireDB, "productStats", pid), {
            title: v.title,
...
        ops.push((b) =>
          b.set(doc(fireDB, "customerStats", uid), {
            name: v.name,
            email: v.email,
            totalOrders: increment(v.totalOrders),
...
        ops.push((b) => b.update(doc(fireDB, "products", pid), { stock: increment(-qty) }))
      })

      // I. clear carts for touched users
      cartUsers.forEach((uid) => {
        ops.push((b) => b.set(doc(fireDB, "carts", uid), { items: [], updatedAt: serverTimestamp() }, { merge: true }))
      })

      log(`Committing ${ops.length} operations in batches of ${BATCH_CHUNK}…`)
      await commitOps(ops, setProgress)
      setProgress(100)
      const dist = orders.reduce((a, o) => { a[o.status] = (a[o.status] || 0) + 1; return a }, {})
      log(` Seeded ${ORDER_COUNT} orders across ${Object.keys(daily).length} days.`)
      log(`Flow: delivered ${dist.delivered || 0} · shipped ${dist.shipped || 0} · confirmed ${dist.confirmed || 0} · placed ${dist.placed || 0}`)
    } catch (err) {
      console.error(err)
      setError(err.message || String(err))
...
      const orderSnap = await getDocs(query(collection(fireDB, "orders"), where("_seeded", "==", true)))

      log("Querying seeded orders in user subcollections…")
      const usersSnap = await getDocs(collection(fireDB, "users"))
      const subRefs = []
      for (const u of usersSnap.docs) {
        const subSnap = await getDocs(collection(fireDB, "users", u.id, "orders"))
        subSnap.forEach((d) => { if (d.data()._seeded) subRefs.push(d.ref) })
      }

      log("Querying seeded inventory logs…")
      const logSnap = await getDocs(query(collection(fireDB, "inventoryLogs"), where("_seeded", "==", true)))

      const refs = [...orderSnap.docs.map((d) => d.ref), ...subRefs, ...logSnap.docs.map((d) => d.ref)]
      if (refs.length === 0) {
...
      log(`Deleting ${orderSnap.size} top-level + ${subRefs.length} subcollection orders + ${logSnap.size} inventory logs…`)
      const ops = refs.map((ref) => (b) => b.delete(ref))
      await commitOps(ops, setProgress)
      setProgress(100)
      log(` Deleted ${refs.length} seeded documents.`)
      log("Note: customerStats / analytics / productStats are NOT rolled back (same as a real order system).")
    } catch (err) {
      console.error(err)
      setError(err.message || String(err))
...
      })
      if (missing.length === 0) {
        log(`All ${snap.size} users already have a status — nothing to backfill.`)
        setProgress(100)
        return
      }
      log(`Backfilling status:"active" on ${missing.length} of ${snap.size} users…`)
      const ops = missing.map((d) => (b) => b.update(d.ref, { status: "active" }))
      await commitOps(ops, setProgress)
      setProgress(100)
...
        const created = new Date(now - (i < 2 ? randInt(1, 8) * 3600000 : randInt(1, 6) * 86400000 + randInt(0, 12) * 3600000))
...
        ops.push((b) => b.set(ref, { event: a.event, actor: a.actor, timestamp: Timestamp.fromDate(ts), type: a.type, _seeded: true }))
      })
      log(`Committing ${ops.length} sample docs…`)
      await commitOps(ops, setProgress)
      setProgress(100)
      log(` Seeded ${SAMPLE_SESSIONS.length} upload sessions + ${SAMPLE_ACTIVITY.length} activity entries.`)
    } catch (err) {
      console.error(err)
      setError(err.message || String(err))
...
        const snap = await getDocs(query(collection(fireDB, c), where("_seeded", "==", true)))
...
     UI
  ============================================================ */
  const showProgress = (seeding || deleting || backfilling || seedingUploads || removingUploads) && progress > 0

  return (
    <div style={{ background: "#0f172a", minHeight: "100vh", color: "#fff", fontFamily: "Inter, sans-serif" }}>
      <div className="max-w-3xl mx-auto px-6 py-12">
        {/* Header */}
        <div className="rounded-2xl p-6 mb-6" style={{ background: "#1B1C1C", border: "1px solid #44474C" }}>
          <h1 className="text-2xl font-black"> Firebase Test Data Generator</h1>
          <p className="text-sm mt-1" style={{ color: "#74777D" }}>
            Seeds {ORDER_COUNT} realistic orders across the past ~1.5 years into every collection an order touches. Admin-only utility.
          </p>
        </div>

        {/* Status box */}
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="rounded-xl p-4" style={{ background: "#1B1C1C", border: "1px solid #44474C" }}>
            <p className="text-xs uppercase tracking-wide" style={{ color: "#74777D" }}>Users loaded</p>
            <p className="text-2xl font-bold" style={{ color: "#A43B31" }}>{users.length}</p>
          </div>
          <div className="rounded-xl p-4" style={{ background: "#1B1C1C", border: "1px solid #44474C" }}>
            <p className="text-xs uppercase tracking-wide" style={{ color: "#74777D" }}>Products loaded</p>
            <p className="text-2xl font-bold" style={{ color: "#A43B31" }}>{products.length}</p>
          </div>
        </div>

        {/* Error */}
        {error && (
          <div className="rounded-xl p-4 mb-6 text-sm" style={{ background: "rgba(239,68,68,0.12)", border: "1px solid #ef4444", color: "#fca5a5" }}>
            {error}
          </div>
        )}
...
            style={{ background: loadingData ? "#44474C" : "#44474C", color: "#fff", opacity: busy && !loadingData ? 0.5 : 1, cursor: busy ? "not-allowed" : "pointer" }}
          >
...
            className="flex-1 rounded-xl px-5 py-3 font-semibold transition-colors"
            style={{ background: "#A43B31", color: "#fff", opacity: busy || !dataLoaded ? 0.5 : 1, cursor: busy || !dataLoaded ? "not-allowed" : "pointer" }}
          >
...
            disabled={busy}
            className="flex-1 rounded-xl px-5 py-3 font-semibold transition-colors"
            style={{ background: "#7f1d1d", color: "#fff", opacity: busy ? 0.5 : 1, cursor: busy ? "not-allowed" : "pointer" }}
          >
...
            disabled={busy}
            className="flex-1 rounded-xl px-5 py-3 font-semibold transition-colors"
            style={{ background: "#065f46", color: "#fff", opacity: busy ? 0.5 : 1, cursor: busy ? "not-allowed" : "pointer" }}
          >
            {backfilling ? `Backfilling… ${progress}%` : 'Backfill Missing User Status → "active"'}
          </button>
        </div>

        {/* Upload dashboard sample data — seed + remove */}
        <div className="rounded-xl p-4 mb-6" style={{ background: "#1B1C1C", border: "1px solid #44474C" }}>
          <p className="text-sm font-semibold mb-1">Upload Dashboard Sample Data</p>
          <p className="text-xs mb-3" style={{ color: "#74777D" }}>
            Populates <code style={{ color: "#F6F3F2" }}>uploadSessions</code> + <code style={{ color: "#F6F3F2" }}>activityLog</code> so the Bulk Order Import page (/admin/adminUploadOrders) shows History, Analytics and Activity. Tagged <code style={{ color: "#F6F3F2" }}>_seeded:true</code>.
          </p>
...
              disabled={busy}
              className="flex-1 rounded-xl px-5 py-3 font-semibold transition-colors"
              style={{ background: "#A43B31", color: "#fff", opacity: busy ? 0.5 : 1, cursor: busy ? "not-allowed" : "pointer" }}
            >
              {seedingUploads ? `Seeding… ${progress}%` : "Seed Upload Sessions + Activity"}
            </button>
            <button
...
              style={{ background: "#7f1d1d", color: "#fff", opacity: busy ? 0.5 : 1, cursor: busy ? "not-allowed" : "pointer" }}
            >
              {removingUploads ? `Removing… ${progress}%` : "Remove Seeded Upload Data"}
            </button>
          </div>
        </div>
...
        {/* Progress bar */}
        {showProgress && (
          <div className="rounded-full overflow-hidden mb-6" style={{ height: "10px", background: "#44474C" }}>
            <div style={{ height: "100%", width: `${progress}%`, background: (deleting || removingUploads) ? "#ef4444" : backfilling ? "#10b981" : seedingUploads ? "#A43B31" : "#A43B31", transition: "width 0.2s" }} />
          </div>
        )}

        {/* Note */}
        <div className="rounded-xl p-4 mb-6 text-xs" style={{ background: "#1B1C1C", border: "1px solid #44474C", color: "#74777D" }}>
          Delete removes seeded <code style={{ color: "#F6F3F2" }}>orders</code> and <code style={{ color: "#F6F3F2" }}>inventoryLogs</code> (tagged <code style={{ color: "#F6F3F2" }}>_seeded: true</code>). Aggregate collections — <code style={{ color: "#F6F3F2" }}>customerStats</code>, <code style={{ color: "#F6F3F2" }}>analytics</code>, <code style={{ color: "#F6F3F2" }}>productStats</code> — and product stock are NOT rolled back, mirroring how a real order system never reverses historical aggregates.
        </div>

        {/* Log panel */}
        <div className="rounded-xl p-4" style={{ background: "#020617", border: "1px solid #44474C" }}>
          <p className="text-xs uppercase tracking-wide mb-2" style={{ color: "#74777D" }}>Activity log (last 20)</p>
          <div className="font-mono text-xs flex flex-col gap-1" style={{ maxHeight: "260px", overflowY: "auto" }}>
            {logs.length === 0 ? (
              <span style={{ color: "#44474C" }}>No activity yet.</span>
            ) : (
              logs.map((l, i) => (
                <span key={i} style={{ color: l.includes("ERROR") ? "#fca5a5" : l.includes(" ") || l.includes(" ") ? "#86efac" : "#cbd5e1" }}>{l}</span>
              ))
            )}
          </div>
        </div>
