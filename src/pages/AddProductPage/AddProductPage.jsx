...
    // Keeps rendering the last product's data while RightDrawer plays its
...
    /* ---------------- DRAWER ---------------- */
    // Animation/backdrop/close (button, outside click, Escape) now live in the shared RightDrawer.
...
    const galleryImagesForDrawer = displayedProduct ? [displayedProduct.thumbnail, ...(displayedProduct.gallery || [])].filter(Boolean) : []
...
                                        const img = imgs[i]
                                        return (
                                            <div key={i} className="flex items-center justify-center overflow-hidden flex-shrink-0 w-[72px] h-[72px] sm:w-[92px] sm:h-[92px]" style={{ borderRadius: "8px", background: C.chip, border: img ? (i === 0 ? `2px solid ${C.indigo}` : `1px solid ${C.border}`) : `1px dashed var(--color-border-strong)` }}>
...
                                Download the template, fill in your product data using the same column headers, then upload it here. Remove the example row before uploading.
                            </p>
                        </div>

                        <label className="flex flex-col items-center justify-center text-center cursor-pointer rounded-xl" style={{ border: `1px dashed color-mix(in srgb, var(--color-error) 40%, transparent)`, background: "color-mix(in srgb, var(--color-error) 30%, transparent)", padding: "28px 16px", gap: "8px" }}>
...
                                        <div key={i} className="grid grid-cols-2 text-[13px]" style={{ background: r.valid ? "color-mix(in srgb, var(--color-success) 15%, transparent)" : "color-mix(in srgb, var(--color-error) 40%, transparent)", borderTop: `1px solid ${C.border}` }}>
...
                        <button onClick={startBatchUpload} disabled={loading || csvRows.length === 0} className="w-full py-3 font-bold text-inverse rounded-lg shadow-md hover:bg-[var(--color-primary)] transition-colors" style={{ background: C.indigo, opacity: loading || csvRows.length === 0 ? 0.5 : 1 }}>
...
                            </div>
                            <div className="relative">
                                <button onClick={() => setShowFilters((s) => !s)} className={`${secBtn} flex items-center gap-2 px-3 py-2 text-sm`} style={(categoryFilter !== "all" || statusFilter !== "all") ? { borderColor: C.indigo, color: C.indigo } : {}}>
                                    <Filter size={15} /> Filters{(categoryFilter !== "all" || statusFilter !== "all") ? " •" : ""}
                                </button>
                                {showFilters && (
                                    <div className="absolute right-0 mt-2 z-30 p-4 flex flex-col gap-3" style={{ width: "240px", background: "var(--color-surface)", border: `1px solid ${C.border}`, borderRadius: "12px", boxShadow: "0 8px 28px rgba(0,0,0,0.12)" }}>
                                        <div>
                                            <label className="block text-[12px] font-bold uppercase tracking-wide mb-1.5" style={{ color: C.textS }}>Category</label>
                                            <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setPage(1) }} className="w-full rounded-lg px-3 py-2 text-sm" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
...
                                            <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1) }} className="w-full rounded-lg px-3 py-2 text-sm" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
...
                            {catalogLoading ? (
                                [...Array(8)].map((_, i) => (
                                    <div key={i} className="border rounded-xl overflow-hidden" style={{ borderColor: C.border }} aria-hidden="true">
                                        <div className="animate-pulse" style={{ height: "140px", background: C.chip }} />
                                        <div className="p-3">
                                            <div className="animate-pulse mb-2" style={{ width: "80%", height: "14px", borderRadius: "4px", background: C.chip }} />
                                            <div className="animate-pulse" style={{ width: "50%", height: "12px", borderRadius: "4px", background: C.chip }} />
                                            <div className="flex items-center justify-between mt-2">
                                                <div className="animate-pulse" style={{ width: "40%", height: "14px", borderRadius: "4px", background: C.chip }} />
                                                <div className="animate-pulse" style={{ width: "28px", height: "18px", borderRadius: "9999px", background: C.chip }} />
                                            </div>
                                            <div className="flex items-center gap-1.5 mt-3">{[...Array(3)].map((_, j) => <div key={j} className="animate-pulse flex-1" style={{ height: "28px", borderRadius: "8px", background: C.chip }} />)}</div>
                                        </div>
                                    </div>
                                ))
                            ) : pageItems.length === 0 ? (
                                <p className="col-span-full py-8 text-center" style={{ color: C.textS }}>No products found.</p>
                            ) : null}
                        </div>
                    )}
...
                                const n = i + 1
                                return (
                                    <button key={n} onClick={() => setPage(n)} className="w-9 h-9 rounded-lg text-sm font-bold" style={page === n ? { background: C.indigo, color: "var(--color-inverse)" } : { border: `1px solid ${C.border}` }}>{n}</button>
                                )
                            })}
...
            {/* DETAIL DRAWER */}
            <RightDrawer open={!!selected} onClose={closeDrawer} title="Product Detail">
                {displayedProduct && (
                    <>
                        {/* Gallery */}
...
                                <p className="font-bold text-[20px]">{money(displayedProduct.price)}</p>
                            </div>
