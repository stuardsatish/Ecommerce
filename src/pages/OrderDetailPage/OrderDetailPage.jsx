...
const unitPrice = (p) =>
  Number(p?.finalPrice ?? p?.price ?? p?.originalPrice ?? 0);
...
const OrderDetailPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const data = { id: snap.id, ...snap.data() }; // Enrich line items with fresh product thumbnails.
        const products = await Promise.all(
          (data.products || []).map(async (item) => {
            if (!item.productId) return item;
            try {
              const pSnap = await getDoc(
                doc(fireDB, "products", item.productId),
              );
              if (pSnap.exists()) {
                const p = pSnap.data();
  ...
  const downloadInvoice = () =>
    generateInvoice(order, {
      ...
      phone: order.userPhone || user?.phone,
  ...
  const computedItems = items.reduce(
    (s, p) => s + unitPrice(p) * Number(p.quantity || 0),
    0,
  );
  const itemsTotal =
    order.subtotal != null ? Number(order.subtotal) : computedItems;
  const shipping =
    order.shipping != null
      ? Number(order.shipping)
      : Math.max(0, Number(order.total || 0) - itemsTotal); // GST is included in the item prices; show the breakdown when the order stored it.
  ...
  return (
    <Shell>
      ...
      <button
        ...
        style={{ gap: "8px", color: "var(--color-body)", marginBottom: "24px" }}
      >
                <ArrowLeft size={16} />{" "}
      ...
      >
               {" "}
        <h2 style={{ fontWeight: 700, fontSize: "16px", marginBottom: "16px" }}>
          Items ({items.length})
        </h2>
               {" "}
          ...
          {items.map((p, i) => (
            <div
              ...
              style={{
                gap: "16px",
                borderTop:
                  i === 0 ? "none" : "1px solid var(--color-info-subtle)",
            ...
            >
              ...
              <div className="flex flex-col flex-1" style={{ minWidth: 0 }}>
                ...
                <span style={{ fontSize: "13px", color: "var(--color-body)" }}>
                  Qty {p.quantity} · {cur(unitPrice(p))} each
                </span>
                             {" "}
              </div>
                           {" "}
              ...
              >
                {cur(unitPrice(p) * Number(p.quantity || 0))}
              </span>
                         {" "}
            </div>
          ))}
      ...
      <div
        className="flex"
        style={{ gap: "24px", marginTop: "24px", flexWrap: "wrap" }}
      >
               {" "}
        <div
          style={{
        ...
        >
                   {" "}
          <h2
            style={{ fontWeight: 700, fontSize: "16px", marginBottom: "16px" }}
          >
            Summary
          </h2>
                   {" "}
          <div className="flex flex-col" style={{ gap: "10px" }}>
            ...
            <div className="flex" style={{ justifyContent: "space-between" }}>
              <span style={{ color: "var(--color-body)" }}>Items</span>
              <span style={{ fontWeight: 600 }}>{cur(itemsTotal)}</span>
            </div>
                       {" "}
            {hasGst &&
              (isInterState ? (
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col">
              ...
                           {" "}
              <span style={{ color: "var(--color-ink)", marginTop: "2px" }}>
