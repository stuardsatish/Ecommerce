...
    try {
      const { data: rows, error } = await supabase.from("promo_codes").select("*");
...
      const fetched = mapProductRows(productRows);
      setProducts(fetched);
...
      // Fetch wishlists and aggregate (per-product distinct user count)
...
        wishlistRows.forEach(({ product_id: pId, user_id: uId }) => {
          if (pId && uId) {
            if (!wishlistCountsMap[pId]) wishlistCountsMap[pId] = new Set();
...
      // Fetch carts and aggregate (per-product distinct user count)
...
        if (error) throw error;
        const cartCountsMap = {};
        cartRows.forEach(({ product_id: pId, user_id: uId }) => {
          if (pId && uId) {
            if (!cartCountsMap[pId]) cartCountsMap[pId] = new Set();
...
    let targets = [];
    if (selectionMode === "individual") targets = getFilteredProducts();
    else if (selectionMode === "wishlist") targets = getFilteredWishlistedProducts();
    else if (selectionMode === "cart") targets = getFilteredCartedProducts();
    const updated = new Set(selectedProductIds);
...
    try {
      setSaving(true);
      const productIds = targets.map((t) => t.id);
      const { error } = await supabase
        .from("products")
        .update({ discount: pct, discount_expiry: expiryVal })
        .in("id", productIds);
      if (error) throw error;

      // Carts join products live at render/fetch time (see src/App.jsx), so
      // there's no cart snapshot to patch here the way Firestore required.
...
  // FIX: Store expiryDate as a proper ISO string so the server-side
  // expiry check (new Date(expiryDate) < Date.now()) always works correctly
  // regardless of timezone. The datetime-local input gives a local time string
  // without timezone; new Date(...).toISOString() converts it to UTC ISO format.
...
      const expiryISO = new Date(newPromoExpiry).toISOString();
...
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex-1 h-3 rounded bg-surface-muted animate-pulse" />
              ))}
            </div>
            {[...Array(4)].map((_, r) => (
              <div key={r} className="flex items-center gap-4 py-4 border-b border-border">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="flex-1 h-4 rounded bg-surface-muted animate-pulse" />
                ))}
              </div>
            ))}
          </div>
...
              Apply discount promotions to products, category-wide, or to the entire inventory
              instantly. Set a timer and the offers revert to standard MRP upon expiration.
            </p>
          </div>
        </div>
...
                                <Heart size={10} className="fill-error" /> Wishlisted by{" "}
                                {p.userCount} {p.userCount === 1 ? "user" : "users"}
                              </span>
                            </div>
                          </div>
                        </div>
...
                                <ShoppingCart size={10} /> In cart for {p.userCount}{" "}
...
              <AlertCircle className="text-accent-strong flex-shrink-0 mt-0.5" size={18} />
...
                  {selectionMode === "all" && (
                    <span>
                      Applying bulk discount to <strong>all products</strong> in the database.
                    </span>
                  )}
                  {selectionMode === "category" && (
                    <span>
                      Applying bulk discount to all products in category:{" "}
...
                      <strong>{selectedProductIds.size}</strong> individually selected product(s).
                    </span>
                  )}
                  {selectionMode === "wishlist" && (
                    <span>
                      Applying bulk discount to{" "}
                      <strong>{selectedProductIds.size}</strong> selected wishlisted product(s).
                    </span>
                  )}
                  {selectionMode === "cart" && (
                    <span>
                      Applying bulk discount to{" "}
                      <strong>{selectedProductIds.size}</strong> selected carted product(s).
                    </span>
                  )}
                  {discountVal && expiryVal ? (
                    <span>
                      {" "}
                      This will apply a promotion of <strong>{discountVal}%</strong> until{" "}
                      <strong>{new Date(expiryVal).toLocaleString()}</strong>. A total of{" "}
                      <strong>{selectedTargetsCount}</strong> product(s) will be updated.
                    </span>
                  ) : (
                    <span>
                      {" "}
                      Please enter a discount percentage and expiry date to compute promotion
                      details.
                    </span>
                  )}
                </p>
...
                <Ticket className="text-accent-strong" size={20} /> Promo Codes &amp; Coupons
                Manager
              </h2>
              <p className="text-muted text-sm mt-1">
                Create coupon codes that apply a percentage discount on the user's overall order
                value. Set an expiration timer; manually deleting a coupon invalidates it
                immediately.
              </p>
            </div>
