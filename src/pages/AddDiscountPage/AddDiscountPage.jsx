...
  if (loading) {
    return (
      <div className="min-h-screen bg-[var(--color-background)] py-12 px-4 sm:px-6 lg:px-8 font-sans" aria-busy="true">
...
              {[...Array(5)].map((_, i) => <div key={i} className="flex-1 h-3 rounded bg-surface-muted animate-pulse" />)}
            </div>
            {[...Array(4)].map((_, r) => (
              <div key={r} className="flex items-center gap-4 py-4 border-b border-border">
                {[...Array(5)].map((_, i) => <div key={i} className="flex-1 h-4 rounded bg-surface-muted animate-pulse" />)}
              </div>
            ))}
          </div>
        </div>
...
                                <Heart size={10} className="fill-error" /> Wishlisted by {p.userCount} {p.userCount === 1 ? 'user' : 'users'}
                              </span>
                            </div>
                          </div>
                        </div>
...
                <Ticket className="text-accent-strong" size={20} /> Promo Codes & Coupons Manager
              </h2>
...
                        return (
                          <tr key={promo.id} className="hover:bg-surface-muted/50 transition-colors">
