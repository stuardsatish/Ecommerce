...
const AddDiscountPage = () => {
  ...
  const handleApplyDiscount = async () => {
    ...
    try {
      ...
      const updateMap = {};
      targets.forEach((t) => {
        updateMap[t.id] = { discount: pct, discountExpiry: expiryVal };
      });
      const batch = writeBatch(fireDB);
      targets.forEach((p) => {
        batch.update(doc(fireDB, "products", p.id), {
          discount: pct,
      ...
      for (const cartDoc of cartsSnapshot.docs) {
        ...
        if (changed) await updateDoc(cartDoc.ref, { items: updatedItems });
      }
  ...
  const handleGeneratePromo = async () => {
    ...
    try {
      ...
      await setDoc(doc(fireDB, "promoCodes", code), {
        code,
        type: "percent",
        value: pct,
  ...
  const handleDeletePromo = async (code) => {
    ...
    try {
      setPromoLoading(true);
      await deleteDoc(doc(fireDB, "promoCodes", code));
