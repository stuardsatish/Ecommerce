...
const GRADIENTS = [
  ...
  [
    "var(--color-chart-6)",
    "color-mix(in srgb, var(--color-chart-6) 55%, white)",
  ],
...
const UsersPage = () => {
  ...
  const containerRef =
    useRef(null); /* ---------------- REALTIME USERS + KPIs ---------------- */
  ...
  const showingFrom =
    filteredUsers.length === 0 ? 0 : (safePage - 1) * ITEMS_PER_PAGE + 1;
...
     `customerStats/{uid}` aggregate), not a /users/{id}/orders subcollection. */
  ...
  const fetchUserStats = async (userId) => {
    try {
        ...
        setSelectedUser((prev) =>
          prev && prev.id === userId
            ? {
                ...
                orderCount: Number(s.totalOrders || 0),
              }
            : prev,
        );
        return;
      ...
      const ordersSnap = await getDocs(
        query(collection(fireDB, "orders"), where("userId", "==", userId)),
      );
      let total = 0;
      ...
      setSelectedUser((prev) =>
        prev && prev.id === userId
          ? { ...prev, totalSpent: total, orderCount: ordersSnap.size }
          : prev,
      );
    } catch (e) {
  ...
  useEffect(() => {
    if (drawerOpen && drawerRef.current) {
      gsap.fromTo(
        drawerRef.current,
        { right: -400 },
        { right: 0, duration: 0.35, ease: "power3.out" },
      );
    }
  }, [drawerOpen]); /* ---------------- ADMIN ACTIONS ---------------- */
  ...
  const suspendUser = async (user) => {
    try {
      ...
      console.log(e);
      toast.error("Could not suspend account.");
    }
  };
  ...
  const resetKeys = async (user) => {
    ...
    // Fixed to genuinely dispatch via Firebase Auth built-in reset email and only toast
    // success once that call resolves; failures surface as errors.
    if (!user.email) {
      toast.error(
        "Cannot reset security keys: no email on file for this user.",
      );
      return;
    }
    try {
      ...
      console.error("resetKeys: sendPasswordResetEmail failed", {
        recipient: user.email,
        code: e.code,
        message: e.message,
      });
      toast.error(
        `Could not send reset email to ${user.email}: ${e.message || "unknown error"}`,
      );
    }
  }; /* ---------------- CSV EXPORT ---------------- */
  ...
  const exportCSV = () => {
    ...
    const rows = filteredUsers.map((u) =>
      [
        ...
        statusOf(u),
        fmtDate(u.createdAt),
        u.lastLoginAt != null ? relativeTime(u.lastLoginAt) : "Never",
        networkIdOf(u),
      ]
        .map(esc)
        .join(","),
  ...
  const menuStyle = {
    ...
    boxShadow: "0px 12px 32px rgba(29,26,35,0.12)",
    padding: "8px",
    fontFamily: INTER,
  };
  const menuItem =
    "w-full text-left px-3 py-2 rounded-lg text-sm hover:bg-[var(--color-surface-muted)] transition-colors capitalize"; /* ============================== LOADING ============================== */
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ ...glassCard, borderRadius: "16px", padding: "16px" }}
          >
                       {" "}
            <div
          ...
          <div
            style={{
              background: "var(--color-surface)",
              border:
                "1px solid color-mix(in srgb, var(--color-border) 10%, transparent)",
              boxShadow: "0px 8px 32px rgba(139,92,246,0.08)",
              borderRadius: "32px",
              padding: "32px",
            }}
          >
            ...
            >
                           {" "}
              <div style={{ minWidth: "232px", flex: 2 }}>
            ...
            {[...Array(8)].map((_, r) => (
              ...
              >
                ...
                <div
                  className="flex items-center"
                  style={{ gap: "16px", minWidth: "232px", flex: 2 }}
                >
                                   {" "}
                  {bar({
                    width: "48px",
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          marginRight: drawerOpen ? "424px" : "0px",
        }}
      >
               {" "}
        ...
        >
          ...
          <div className="flex flex-wrap items-center" style={{ gap: "12px" }}>
            ...
            <div className="relative">
              ...
              {openMenu === "date" && (
                <div
                  style={{ ...menuStyle, left: 0, minWidth: "240px" }}
            ...
            <div className="flex items-center" style={{ gap: "8px" }}>
              ...
              <span
                style={{
                  ...
                  boxShadow:
                    "0px 0px 8px color-mix(in srgb, var(--color-success) 60%, transparent)",
                  borderRadius: "4px",
                }}
              />
                           {" "}
              ...
              <tbody>
                ...
                {paginatedUsers.map((u) => {
                  ...
                  return (
                    <tr
                      key={u.id}
                      className="table-row cursor-pointer transition-colors hover:bg-[var(--color-surface-muted)]"
                      style={{
                        borderTop:
                          "1px solid color-mix(in srgb, var(--color-border) 10%, transparent)",
                      }}
                        ...
                        >
                          ...
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: "9999px",
                              border:
                                "2px solid color-mix(in srgb, var(--color-accent) 10%, transparent)",
            ...
            <div className="flex items-center" style={{ gap: "8px" }}>
              ...
              {buildPages(safePage, totalPages).map((p, i) =>
                p === "…" ? (
                  <span
                    key={`e${i}`}
                    style={{ color: "var(--color-muted)", padding: "0 4px" }}
                  >
                    …
                  </span>
                ) : (
      ...
      <div
        ...
        style={{
          ...
          background: "var(--color-surface)",
          zIndex: 45,
          boxShadow: "-12px 0 40px rgba(29,26,35,0.12)",
          display: "flex",
          flexDirection: "column",
        }}
      >
          ...
          <>
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "9999px",
                    border:
                      "4px solid color-mix(in srgb, var(--color-accent) 10%, transparent)",
                    background: selectedUser.photoURL
                      ? "transparent"
                      : gradientFor(selectedUser.name),
                  }}
                >
                                   {" "}
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      toast.info("Messaging is not configured yet.")
                    }
                    className="flex items-center justify-center"
                    style={{
                      background:
                  ...
                                   {" "}
                  <button
                    onClick={() => toast.info("No phone number on file.")}
                    className="flex items-center justify-center"
                    style={{
                      background:
                        "color-mix(in srgb, var(--color-accent) 10%, transparent)",
              ...
              <div className="flex flex-col" style={{ gap: "24px" }}>
                ...
                {(() => {
                  ...
                  const rows = [
                    {
                      label: "Network ID",
                      value: networkIdOf(selectedUser),
                      color: "var(--color-ink)",
                      font: INTER,
                      weight: 600,
                    },
              ...
              <div className="flex flex-col" style={{ gap: "16px" }}>
                ...
                               {" "}
                <button
                  onClick={() => resetKeys(selectedUser)}
