// ============================================================================
// payment-create-order — SELF-CONTAINED Dashboard bundle.
// Paste this whole file as the function's index.ts in the Supabase Dashboard
// (Edge Functions -> Deploy a new function -> name it "payment-create-order").
// Equivalent to supabase/functions/payment-create-order/index.ts + its _shared/*
// deps, inlined into one file so nothing outside this file is needed.
// ============================================================================
...
async function getBuyerState(admin: SupabaseClient, userId: string): Promise<string> {
  try {
    const { data, error } = await admin.from("profiles").select("address").eq("id", userId).maybeSingle();
    if (error || !data) return "";
    return data.address?.state || "";
  } catch (e) { console.warn("[pricing] buyer state lookup failed:", e); return ""; }
}
...
/* ============================== razorpay.ts (getRazorpay only) ============================== */
let _razorpay: Razorpay | null = null;
function getRazorpay(): Razorpay {
  if (_razorpay) return _razorpay;
  const key_id = Deno.env.get("RAZORPAY_KEY_ID");
  const key_secret = Deno.env.get("RAZORPAY_KEY_SECRET");
  if (!key_id || !key_secret) throw new Error("Missing RAZORPAY_KEY_ID / RAZORPAY_KEY_SECRET env vars");
